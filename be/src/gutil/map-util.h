...
typename Collection::mapped_type& FindFloorOrDie(
    Collection& collection, const typename Collection::key_type& key) {
  auto it = collection.upper_bound(key);
  CHECK(it != collection.begin());
  return (--it)->second;
}
...
// unique_ptr.
// Returns the raw pointer contained in the smart pointer for the first found key, if it
// exists,
...
void AddTokenCounts(const Sequence& sequence,
    const typename Collection::mapped_type& increment, Collection* const count_map) {
...
// Compute and insert new value if it's absent from the map. Return a pair with a
// reference to the
...
//
// It takes a reference to the key and a lambda function. If the key exists in the map,
// returns
// a pair with a pointer to the current value and 'false'. If the key does not exist in
// the map,
...
pair<typename MapContainer::mapped_type* const, bool> ComputeIfAbsentReturnAbsense(
  ...
  if (new_value) {
    pair<typename MapContainer::iterator, bool> result =
...
typename MapContainer::mapped_type* const ComputeIfAbsent(MapContainer* container,
    const typename MapContainer::key_type& key, Function compute_func) {
