...
class LazyCpuInfoValue {
 public:
  LazyCpuInfoValue() : has_broken_neon_(false) {
    ...
    unsigned implementer = 0, architecture = 0, variant = 0, part = 0, revision = 0;
    const struct {
      const char key[17];
      unsigned int* result;
    } kUnsignedValues[] = {
        {"CPU implementer", &implementer}, {"CPU architecture", &architecture},
        {"CPU variant", &variant}, {"CPU part", &part}, {"CPU revision", &revision},
    };
    ...
    while (std::getline(iss, line)) {
      ...
      for (size_t i = 0; i < arraysize(kUnsignedValues); i++) {
        ...
        if (line.compare(0, len, key) == 0 && line.size() >= len + 1
          ...
          while (!value_sp.empty() && (value_sp[0] == ' ' || value_sp[0] == '\t')) {
    ...
    has_broken_neon_ = implementer == 0x51 && architecture == 7 && variant == 1
...
void CPU::Initialize() {
  ...
  if (cpu_info[0] >= parameter_end) {
    ...
    for (int parameter = 0x80000002;
         parameter <= parameter_end && cpu_string_ptr < &cpu_string[sizeof(cpu_string)];
         parameter++) {
