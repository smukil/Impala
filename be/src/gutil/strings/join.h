...
void JoinKeysIterator(const ITERATOR& start, const ITERATOR& end,
    const StringPiece& delim, string* result) {
  result->clear();
...
template <typename ITERATOR>
string JoinKeysIterator(
    const ITERATOR& start, const ITERATOR& end, const StringPiece& delim) {
  string result;
...
string JoinMapped(
    const CONTAINER& components, const FUNC& functor, const StringPiece& delim) {
  string result;
