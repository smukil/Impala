...
void FragmentInstanceState::GetStatusReport(FragmentInstanceExecStatusPB* instance_status,
    vector<TRuntimeProfileTree>* profiles_list, ReportExecStatusRequestPB* report) {
  ...
  finstance_id_pb->set_lo(instance_id().lo);
  finstance_id_pb->set_hi(instance_id().hi);

  instance_status->set_done(IsDone());
  instance_status->set_current_state(current_state());
  DCHECK(profile() != nullptr);

  profiles_list->push_back(TRuntimeProfileTree());
  TRuntimeProfileTree* thrift_profile = &profiles_list->back();
  ...
  // TODO: We shouldn't overwrite 'report->mutable_insert_exec_status()'.
  //if (done) state->dml_exec_state()->ToProto(report->mutable_insert_exec_status());
