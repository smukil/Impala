...
Striped64::Striped64()
  : busy_(false), cell_buffer_(nullptr), cells_(nullptr), num_cells_(0) {}
...
void Striped64::RetryUpdate(int64_t x, Rehash contention) {
  ...
  //
  // These are predicated on successful CAS operations, which is why it's all wrapped in
  // an
  // infinite retry loop.
  while (true) {
...
void LongAdder::IncrementBy(int64_t x) {
  ...
  // If no hash table, try to CAS the base counter. If that fails, RetryUpdate to init the
  // table.
  const int32_t n = base::subtle::Acquire_Load(&num_cells_);
  if (n > 0) {
    ...
    if (!base_.CompareAndSet(b, b + x)) {
      // Attempt to initialize the table. No need to rehash since the contention was for
      // the
