...
#define RETRY_ON_EINTR(err, expr)                                                        \
  do {                                                                                   \
    static_assert(                                                                       \
        std::is_signed<decltype(err)>::value == true, #err " must be a signed integer"); \
    (err) = (expr);                                                                      \
...
DEFINE_bool(suicide_on_eio, true, "Kill the process if an I/O operation results in EIO");
...
int fallocate(int fd, int mode, off_t offset, off_t len) {
  ...
  if (stat.st_blocks * 512 < size) {
    ...
    if (fcntl(fd, F_PREALLOCATE, &store) < 0) {
      LOG(INFO) << "Unable to allocate contiguous disk space, attempting non-contiguous "
                   "allocation";
...
class PosixRandomAccessFile : public RandomAccessFile {
 ...
 public:
  ...
  virtual Status Read(
      uint64_t offset, size_t n, Slice* result, uint8_t* scratch) const OVERRIDE {
    ThreadRestrictions::AssertIOAllowed();
    Status s;
    ssize_t r;
...
class PosixWritableFile : public WritableFile {
 public:
  ...
  virtual uint64_t Size() const OVERRIDE { return filesize_; }
...
class PosixRWFile : public RWFile {
 public:
  ...
  virtual Status Read(
      uint64_t offset, size_t length, Slice* result, uint8_t* scratch) const OVERRIDE {
    ThreadRestrictions::AssertIOAllowed();
    int rem = length;
    uint8_t* dst = scratch;
    while (rem > 0) {
      ...
      if (this_result.size() == 0) {
        // EOF
        return Status::IOError(
            Substitute("EOF trying to read $0 bytes at offset $1", length, offset));
      }
  ...
  virtual Status Truncate(uint64_t length) OVERRIDE {
    ...
    if (ret != 0) {
      ...
      return Status::IOError(Substitute("Unable to truncate file $0", filename_),
          Substitute("ftruncate() failed: $0", ErrnoToString(err)), err);
    }
    return Status::OK();
  }
...
class PosixEnv : public Env {
 public:
  ...
  virtual Status NewSequentialFile(
      const std::string& fname, gscoped_ptr<SequentialFile>* result) OVERRIDE {
  ...
  virtual Status NewRandomAccessFile(
      const std::string& fname, gscoped_ptr<RandomAccessFile>* result) OVERRIDE {
  ...
  virtual Status NewWritableFile(
      const std::string& fname, gscoped_ptr<WritableFile>* result) OVERRIDE {
  ...
  virtual Status NewTempWritableFile(const WritableFileOptions& opts,
      const std::string& name_template, std::string* created_filename,
  ...
  virtual Status NewRWFile(const string& fname, gscoped_ptr<RWFile>* result) OVERRIDE {
    return NewRWFile(RWFileOptions(), fname, result);
  }

  virtual Status NewRWFile(const RWFileOptions& opts, const string& fname,
  ...
  virtual Status NewTempRWFile(const RWFileOptions& opts,
      const std::string& name_template, std::string* created_filename,
      gscoped_ptr<RWFile>* res) OVERRIDE {
  ...
  virtual Status GetChildren(
      const std::string& dir, std::vector<std::string>* result) OVERRIDE {
    ...
    struct dirent* entry;
    // TODO: lint: Consider using readdir_r(...) instead of readdir(...) for improved
    // thread safety.
  ...
  virtual Status DeleteRecursively(const std::string& name) OVERRIDE {
    return Walk(name, POST_ORDER, Bind(&PosixEnv::DeleteRecursivelyCb, Unretained(this)));
  }
  ...
  virtual Status GetFileSizeOnDiskRecursively(
      const string& root, uint64_t* bytes_used) OVERRIDE {
    TRACE_EVENT1("io", "PosixEnv::GetFileSizeOnDiskRecursively", "path", root);
    uint64_t total = 0;
    RETURN_NOT_OK(Walk(root, Env::PRE_ORDER,
        Bind(&PosixEnv::GetFileSizeOnDiskRecursivelyCb, Unretained(this), &total)));
    *bytes_used = total;
    return Status::OK();
  }
...
#error Unsupported platform
  ...
  virtual Status Walk(
      const string& root, DirectoryOrder order, const WalkCallback& cb) OVERRIDE {
 ...
 private:
  ...
  Status MkTmpFile(const string& name_template, int* fd, string* created_filename) {
    ...
    if (created_fd < 0) {
      return IOError(
          Substitute("Call to mkstemp() failed on name template $0", name_template),
          errno);
    }
    *fd = created_fd;
  ...
  Status InstantiateNewWritableFile(const std::string& fname, int fd,
      const WritableFileOptions& opts, gscoped_ptr<WritableFile>* result) {
  ...
  Status DeleteRecursivelyCb(
      FileType type, const string& dirname, const string& basename) {
  ...
  Status GetFileSizeOnDiskRecursivelyCb(uint64_t* bytes_used, Env::FileType type,
      const string& dirname, const string& basename) {
    uint64_t file_bytes_used = 0;
    switch (type) {
      case Env::FILE_TYPE:
        RETURN_NOT_OK(
            GetFileSizeOnDisk(JoinPathSegments(dirname, basename), &file_bytes_used));
