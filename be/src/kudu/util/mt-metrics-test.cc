...
DEFINE_int32(
    mt_metrics_test_num_threads, 4, "Number of threads to spawn in mt metrics tests");
...
static void RunWithManyThreads(boost::function<void()>* f, int num_threads) {
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    CHECK_OK(kudu::Thread::Create("test", StringPrintf("thread%d", i), *f, &new_thread));
...
METRIC_DEFINE_counter(
    test_entity, test_counter, "Test Counter", MetricUnit::kRequests, "Test counter");
...
TEST_F(MultiThreadedMetricsTest, CounterIncrementTest) {
  ...
  boost::function<void()> f = boost::bind(CountWithCounter, counter, num_increments);
...
void MultiThreadedMetricsTest::RegisterCounters(
    const scoped_refptr<MetricEntity>& metric_entity, const string& name_prefix,
    int num_counters) {
  ...
  for (int i = 0; i < num_counters; i++) {
    ...
    auto proto = new CounterPrototype(MetricPrototype::CtorArgs("test_entity",
        strdup(name.c_str()), "Test Counter", MetricUnit::kOperations, "test counter"));
