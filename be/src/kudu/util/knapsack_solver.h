...
class KnapsackSolver {
 public:
  ...
  void Solve(std::vector<item_type>& items, int knapsack_capacity,
      std::vector<int>* chosen_items, value_type* optimal_value);
  ...
  void Reset(int knapsack_capacity, const std::vector<item_type>* items);
  ...
  void TracePath(const solution_type& best, std::vector<int>* chosen_items);
 ...
 private:
  ...
  class KnapsackBlackboard {
   ...
   private:
    void MarkTaken(int item, int weight) { item_taken_[index(item, weight)] = true; }

    // If the dynamic programming matrix has more than this number of cells,
    // then warn.
    static const int kWarnDimension = 10000000;

    int index(int item, int weight) const { return n_weights_ * item + weight; }
...
inline void KnapsackSolver<Traits>::Reset(
    int knapsack_capacity, const std::vector<item_type>* items) {
...
inline void KnapsackSolver<Traits>::Solve(std::vector<item_type>& items,
    int knapsack_capacity, std::vector<int>* chosen_items, value_type* optimal_value) {
...
inline void KnapsackSolver<Traits>::TracePath(
    const solution_type& best, std::vector<int>* chosen_items) {
  chosen_items->clear();
