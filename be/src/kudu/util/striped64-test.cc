 ...
 public:
  ...
  void Run() {
    ...
    for (int i = 0; i < num_threads_; i++) {
      ...
      Thread::Create("Striped64", "Incrementer", &MultiThreadTest::IncrementerThread,
          this, num_operations_, &ref);
    ...
    for (int i = 0; i < num_threads_; i++) {
      scoped_refptr<Thread> ref;
      Thread::Create("Striped64", "Decrementer", &MultiThreadTest::DecrementerThread,
          this, num_operations_, &ref);
      threads_.push_back(ref);
    }
    for (const scoped_refptr<Thread>& t : threads_) {
...
TEST(Striped64Test, TestSingleIncrDecr) {
  OverrideFlagForSlowTests(
      "num_operations", strings::Substitute("$0", (FLAGS_num_operations * 100)));
