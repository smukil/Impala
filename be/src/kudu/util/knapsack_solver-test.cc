...
struct TestItemTraits {
  ...
  static int get_weight(const TestItem& item) { return item.weight; }
  static value_type get_value(const TestItem& item) { return item.value; }
};

// Generate random items into the provided vector.
static void GenerateRandomItems(int n_items, int max_weight, vector<TestItem>* out) {
