 ...
 private:
  ...
  struct TLS {
    ...
    struct Data {
      ...
      //
      // Before modifying any data inside its TLS, the watched thread increments this
      // value so it is
      // odd. When the modifications are complete, it increments it again, making it even.
      //
      // To read the TLS data from a target thread, the watchdog thread waits for the
      // value
      // to become even, indicating that no write is in progress. Then, it does a
      // potentially
      ...
      //
      // We use this type of locking to ensure that the watched thread is as fast as
      // possible,
      // allowing us to use SCOPED_WATCH_STACK even in hot code paths. In particular,
      // the watched thread is wait-free, since it doesn't need to loop or retry. In
      // addition, the
      // memory is only written by that thread, eliminating any cache-line bouncing. The
      // watchdog
      // thread may have to loop multiple times to see a consistent snapshot, but we're OK
      // delaying
      // the watchdog arbitrarily since it isn't on any critical path.
      Atomic32 seq_lock_;

      // Take a consistent snapshot of this data into 'dst'. This may block if the target
      // thread
