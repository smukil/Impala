...
class Socket {
 public:
  ...
  // Underlying Socket expected to be in blocking mode. Fails if any Write() sends 0
  // bytes.
  ...
  Status BlockingWrite(
      const uint8_t* buf, size_t buflen, size_t* num_written, const MonoTime& deadline);
