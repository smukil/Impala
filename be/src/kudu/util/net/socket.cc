...
Status Socket::Close() {
  ...
  if (::close(fd) < 0) {
    ...
    return Status::NetworkError(
        std::string("close error: ") + ErrnoToString(err), Slice(), err);
  }
  fd = -1;
  return Status::OK();
}
...
Status Socket::Shutdown(bool shut_read, bool shut_write) {
  ...
  if (::shutdown(fd_, flags) < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("shutdown error: ") + ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::Init(int flags) {
  ...
  if (fd_ < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("error opening socket: ") + ErrnoToString(err), Slice(), err);
  }
...
Status Socket::Init(int flags) {
  ...
  if (setsockopt(fd_, SOL_SOCKET, SO_NOSIGPIPE, &set, sizeof(set)) == -1) {
    int err = errno;
    return Status::NetworkError(
        std::string("failed to set SO_NOSIGPIPE: ") + ErrnoToString(err), Slice(), err);
  }
...
Status Socket::SetNoDelay(bool enabled) {
  ...
  if (setsockopt(fd_, IPPROTO_TCP, TCP_NODELAY, &flag, sizeof(flag)) == -1) {
    int err = errno;
    return Status::NetworkError(
        std::string("failed to set TCP_NODELAY: ") + ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::SetNonBlocking(bool enabled) {
  ...
  if (curflags == -1) {
    int err = errno;
    return Status::NetworkError(
        StringPrintf("Failed to get file status flags on fd %d", fd_), ErrnoToString(err),
        err);
  }
  ...
  if (::fcntl(fd_, F_SETFL, newflags) == -1) {
    int err = errno;
    if (enabled) {
      return Status::NetworkError(StringPrintf("Failed to set O_NONBLOCK on fd %d", fd_),
          ErrnoToString(err), err);
    } else {
      return Status::NetworkError(
          StringPrintf("Failed to clear O_NONBLOCK on fd %d", fd_), ErrnoToString(err),
          err);
    }
  }
  return Status::OK();
...
Status Socket::SetCloseOnExec() {
  ...
  if (curflags == -1) {
    ...
    Reset(-1);
    return Status::NetworkError(
        std::string("fcntl(F_GETFD) error: ") + ErrnoToString(err), Slice(), err);
  }
  if (fcntl(fd_, F_SETFD, curflags | FD_CLOEXEC) == -1) {
    int err = errno;
    Reset(-1);
    return Status::NetworkError(
        std::string("fcntl(F_SETFD) error: ") + ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::SetReuseAddr(bool flag) {
  ...
  if (setsockopt(fd_, SOL_SOCKET, SO_REUSEADDR, &int_flag, sizeof(int_flag)) == -1) {
    err = errno;
    return Status::NetworkError(
        std::string("failed to set SO_REUSEADDR: ") + ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::GetSocketAddress(Sockaddr* cur_addr) const {
  ...
  if (::getsockname(fd_, (struct sockaddr*)&sin, &len) == -1) {
    int err = errno;
    return Status::NetworkError(
        string("getsockname error: ") + ErrnoToString(err), Slice(), err);
  }
  *cur_addr = sin;
  return Status::OK();
}
...
Status Socket::GetPeerAddress(Sockaddr* cur_addr) const {
  ...
  if (::getpeername(fd_, (struct sockaddr*)&sin, &len) == -1) {
    int err = errno;
    return Status::NetworkError(
        string("getpeername error: ") + ErrnoToString(err), Slice(), err);
  }
  *cur_addr = sin;
  return Status::OK();
}
...
Status Socket::Bind(const Sockaddr& bind_addr) {
  ...
  if (PREDICT_FALSE(::bind(fd_, (struct sockaddr*)&addr, sizeof(addr)))) {
    int err = errno;
    Status s = Status::NetworkError(strings::Substitute("error binding socket to $0: $1",
...
Status Socket::Accept(Socket* new_conn, Sockaddr* remote, int flags) {
  ...
  new_conn->Reset(::accept4(fd_, (struct sockaddr*)&addr, &olen, accept_flags));
  if (new_conn->GetFd() < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("accept4(2) error: ") + ErrnoToString(err), Slice(), err);
  }
#else
  ...
  if (new_conn->GetFd() < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("accept(2) error: ") + ErrnoToString(err), Slice(), err);
  }
  ...
  *remote = addr;
  TRACE_EVENT_INSTANT1(
      "net", "Accepted", TRACE_EVENT_SCOPE_THREAD, "remote", remote->ToString());
  return Status::OK();
}
...
Status Socket::Connect(const Sockaddr& remote) {
  ...
  if (::connect(fd_, (const struct sockaddr*)&addr, sizeof(addr)) < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("connect(2) error: ") + ErrnoToString(err), Slice(), err);
  }
  return Status::OK();
}
...
Status Socket::GetSockError() const {
  ...
  if (ret) {
    int err = errno;
    return Status::NetworkError(
        std::string("getsockopt(SO_ERROR) failed: ") + ErrnoToString(err), Slice(), err);
  }
  if (val != 0) {
...
Status Socket::Write(const uint8_t* buf, int32_t amt, int32_t* nwritten) {
  if (amt <= 0) {
    return Status::NetworkError(
        StringPrintf("invalid send of %" PRId32 " bytes", amt), Slice(), EINVAL);
  }
  DCHECK_GE(fd_, 0);
  ...
  if (res < 0) {
    int err = errno;
    return Status::NetworkError(
        std::string("write error: ") + ErrnoToString(err), Slice(), err);
  }
  *nwritten = res;
  return Status::OK();
}

Status Socket::Writev(const struct ::iovec* iov, int iov_len, int32_t* nwritten) {
  if (PREDICT_FALSE(iov_len <= 0)) {
    return Status::NetworkError(
        StringPrintf("writev: invalid io vector length of %d", iov_len), Slice(), EINVAL);
  }
  DCHECK_GE(fd_, 0);
  ...
  if (PREDICT_FALSE(res < 0)) {
    int err = errno;
    return Status::NetworkError(
        std::string("sendmsg error: ") + ErrnoToString(err), Slice(), err);
  }
...
Status Socket::BlockingWrite(
    const uint8_t* buf, size_t buflen, size_t* nwritten, const MonoTime& deadline) {
...
Status Socket::Recv(uint8_t* buf, int32_t amt, int32_t* nread) {
  ...
  if (res <= 0) {
    ...
    int err = errno;
    return Status::NetworkError(
        std::string("recv error: ") + ErrnoToString(err), Slice(), err);
  }
  *nread = res;
  return Status::OK();
}
...
Status Socket::BlockingRecv(
    uint8_t* buf, size_t amt, size_t* nread, const MonoTime& deadline) {
...
Status Socket::SetTimeout(int opt, std::string optname, const MonoDelta& timeout) {
  ...
  if (::setsockopt(fd_, SOL_SOCKET, opt, &tv, optlen) == -1) {
    int err = errno;
    return Status::NetworkError(StringPrintf("Failed to set %s to %s", optname.c_str(),
