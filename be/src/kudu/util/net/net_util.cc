...
struct AddrinfoDeleter {
  void operator()(struct addrinfo* info) { freeaddrinfo(info); }
};
}
...
HostPort::HostPort(const Sockaddr& addr) : host_(addr.host()), port_(addr.port()) {}
...
Status HostPort::ParseString(const string& str, uint16_t default_port) {
  ...
  if (p.second.empty() && strcount(str, ':') == 0) {
    // No port specified.
    port = default_port;
  } else if (!SimpleAtoi(p.second, &port) || port > 65535) {
...
Status HostPort::ResolveAddresses(vector<Sockaddr>* addresses) const {
  ...
  int rc;
  LOG_SLOW_EXECUTION(WARNING, 200, Substitute("resolving address for $0", host_)) {
    rc = getaddrinfo(host_.c_str(), nullptr, &hints, &res);
  }
  if (rc != 0) {
    return Status::NetworkError(
        StringPrintf("Unable to resolve address '%s'", host_.c_str()), gai_strerror(rc));
  }
...
Status HostPort::ParseStrings(
    const string& comma_sep_addrs, uint16_t default_port, vector<HostPort>* res) {
...
Status ParseAddressList(const std::string& addr_list, uint16_t default_port,
...
Status GetFQDN(string* hostname) {
  ...
  LOG_SLOW_EXECUTION(
      WARNING, 200, Substitute("looking up canonical hostname for localhost "
                               "(eventual result was $0)",
                        *hostname)) {
