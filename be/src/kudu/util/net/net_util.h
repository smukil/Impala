...
class HostPort {
 public:
  ...
  // pair.
  static Status ParseStrings(const std::string& comma_sep_addrs, uint16_t default_port,
...
struct HostPortHasher {
  size_t operator()(const HostPort& hp) const { return hp.HashCode(); }
};

// Equality BinaryPredicate of HostPort objects for UnorderedAssociativeContainers.
struct HostPortEqualityPredicate {
  bool operator()(const HostPort& hp1, const HostPort& hp2) const { return hp1 == hp2; }
};
