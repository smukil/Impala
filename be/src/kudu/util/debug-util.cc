...
struct SignalCommunication::Lock {
  ...
  ~Lock() { base::subtle::Release_Store(&g_comm.lock, 0); }
};
...
bool InitSignalHandlerUnlocked(int signum) {
  ...
  if (state == UNINITIALIZED) {
    ...
    if (old_act.sa_handler != SIG_DFL && old_act.sa_handler != SIG_IGN) {
      state = INIT_ERROR;
      LOG(WARNING) << "signal handler for stack trace signal " << g_stack_trace_signum
                   << " is already in use: "
...
string StackTrace::Symbolize() const {
  ...
  for (int i = 0; i < num_frames_; i++) {
    ...
    // on logged stacks.
    if (google::Symbolize(reinterpret_cast<char*>(pc) - 1, tmp, sizeof(tmp))) {
      symbol = tmp;
    }
