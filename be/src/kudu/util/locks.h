...
class simple_spinlock {
 public:
  ...
  bool try_lock() { return l_.TryLock(); }
...
class rw_spinlock {
 public:
  rw_spinlock() { ANNOTATE_RWLOCK_CREATE(this); }
  ~rw_spinlock() { ANNOTATE_RWLOCK_DESTROY(this); }
  ...
  bool is_write_locked() const { return sem_.is_write_locked(); }

  bool is_locked() const { return sem_.is_locked(); }
...
class shared_lock {
 public:
  ...
  explicit shared_lock(Mutex& m) : m_(&m) { m_->lock_shared(); }
  ...
  bool owns_lock() const { return m_; }

  void swap(shared_lock& other) { std::swap(m_, other.m_); }
