...
class ResettableHeartbeaterTest : public KuduTest {
 public:
  ResettableHeartbeaterTest() : KuduTest(), latch_(kNumHeartbeats) {}

 protected:
  void CreateHeartbeater(uint64_t period_ms, const std::string& name) {
    period_ms_ = period_ms;
    heartbeater_.reset(
        new ResettableHeartbeater(name, MonoDelta::FromMilliseconds(period_ms),
            boost::bind(&ResettableHeartbeaterTest::HeartbeatFunction, this)));
  }
