...
class TestEnv : public KuduTest {
 public:
  ...
  void CheckFallocateSupport() {
    ...
    if (err != 0) {
      ...
      err = fallocate(fd, FALLOC_FL_KEEP_SIZE | FALLOC_FL_PUNCH_HOLE, 1024, 1024);
      if (err != 0) {
        PCHECK(errno == ENOTSUP);
      } else {
 ...
 protected:
  ...
  void ReadAndVerifyTestData(RandomAccessFile* raf, size_t offset, size_t n) {
    ...
    Slice s;
    ASSERT_OK(env_util::ReadFully(raf, offset, n, &s, scratch.get()));
    ASSERT_EQ(n, s.size());
...
TEST_F(TestEnv, TestPreallocate) {
  ...
  shared_ptr<WritableFile> file;
  ASSERT_OK(
      env_util::OpenFileForWrite(WritableFileOptions(), env_.get(), test_path, &file));
...
class ShortReadRandomAccessFile : public RandomAccessFile {
 public:
  ...
  virtual Status Size(uint64_t* size) const OVERRIDE { return wrapped_->Size(size); }
...
TEST_F(TestEnv, TestAppendVector) {
  ...
  if (!fallocate_supported_) {
    ...
    LOG(INFO) << "Testing AppendVector() together with Append() and Read(), WITH "
                 "pre-allocation";
...
TEST_F(TestEnv, TestOverwrite) {
  ...
  Status s = env_util::OpenFileForWrite(opts, env_.get(), test_path, &writer);
...
TEST_F(TestEnv, TestReopen) {
  ...
  shared_ptr<WritableFile> writer;
  ASSERT_OK(
      env_util::OpenFileForWrite(WritableFileOptions(), env_.get(), test_path, &writer));
  ...
  ASSERT_OK(env_util::OpenFileForWrite(reopen_opts, env_.get(), test_path, &writer));
  ASSERT_EQ(first.length(), writer->Size());
...
static Status TestWalkCb(vector<string>* actual, Env::FileType type,
...
static Status TestWalkErrorCb(
    int* num_calls, Env::FileType type, const string& dirname, const string& basename) {
  (*num_calls)++;
...
TEST_F(TestEnv, TestWalkCbReturnsError) {
  ...
  int num_calls = 0;
  ASSERT_TRUE(env_->Walk(new_dir, Env::PRE_ORDER, Bind(&TestWalkErrorCb, &num_calls))
                  .IsIOError());
...
TEST_F(TestEnv, TestGetBytesFree) {
  ...
  ASSERT_GE(orig_bytes_free - cur_bytes_free, kFileSizeBytes) << "Failed after " << kIters
                                                              << " attempts";
}
