...
class ObjectPool {
 public:
  ...
  ObjectPool() : free_list_head_(NULL), alloc_list_head_(NULL), deleter_(this) {}
  ...
  void Destroy(T* t) {
    CHECK_NOTNULL(t);
    ListNode* node = static_cast<ListNode*>(reinterpret_cast<ManualConstructor<T>*>(t));
  ...
  // to the pool.
  scoped_ptr make_scoped_ptr(T* ptr) { return scoped_ptr(ptr, deleter_); }
...
class ReturnToPool {
 public:
  ...
  inline void operator()(T* ptr) const { pool_->Destroy(ptr); }
