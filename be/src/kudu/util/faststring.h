...
class faststring {
 public:
  ...
  // If the new length is larger than the old length, the capacity is expanded as
  // necessary.
  //
  // NOTE: in contrast to std::string's implementation, Any newly "exposed" bytes of data
  // are
  // not cleared.
  ...
  // larger than the newly requested capacity, this is a no-op (i.e. it does not ever free
  // memory).
  //
  // NOTE: even though the new capacity is reserved, it is illegal to begin writing into
  // that memory
  // directly using pointers. If ASAN is enabled, this is ensured using manual memory
  // poisoning.
