...
class rw_semaphore {
 public:
  ...
  bool try_lock() {
    ...
    while (true) {
      ...
      if (cur_state & kWriteFlag) return false;
  ...
  void lock() {
    ...
    while (true) {
      ...
      cur_state =
          base::subtle::NoBarrier_CompareAndSwap(&state_, expected, try_new_state);
      if (cur_state == expected) break;
      // Either was already locked by someone else, or CAS failed.
      boost::detail::yield(loop_count++);
    }
  ...
  // by any thread.
  // See simple_semaphore::is_locked() for details about where this is useful.
  bool is_locked() const { return base::subtle::NoBarrier_Load(&state_); }
 ...
 private:
  ...
  void RecordLockHolderStack() { writer_stack_.Collect(); }
  void ResetLockHolderStack() { writer_stack_.Reset(); }
#else
