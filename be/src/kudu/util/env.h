...
class Env {
 public:
  ...
  virtual Status NewSequentialFile(
      const std::string& fname, gscoped_ptr<SequentialFile>* result) = 0;
  ...
  virtual Status NewRandomAccessFile(
      const std::string& fname, gscoped_ptr<RandomAccessFile>* result) = 0;
  ...
  virtual Status NewWritableFile(
      const std::string& fname, gscoped_ptr<WritableFile>* result) = 0;
  ...
  virtual Status NewRWFile(const std::string& fname, gscoped_ptr<RWFile>* result) = 0;

  // Like the previous NewRWFile, but allows options to be specified.
  virtual Status NewRWFile(const RWFileOptions& opts, const std::string& fname,
  ...
  virtual Status GetFileSizeOnDiskRecursively(
      const std::string& root, uint64_t* bytes_used) = 0;
  ...
  virtual Status Walk(
      const std::string& root, DirectoryOrder order, const WalkCallback& cb) = 0;
...
class RandomAccessFile {
 public:
  ...
  virtual Status Read(
      uint64_t offset, size_t n, Slice* result, uint8_t* scratch) const = 0;
...
struct RWFileOptions {
  ...
  RWFileOptions() : sync_on_close(false), mode(Env::CREATE_IF_NON_EXISTING_TRUNCATE) {}
};
...
class RWFile {
 public:
  ...
  // Safe for concurrent use by multiple threads.
  virtual Status Read(
      uint64_t offset, size_t length, Slice* result, uint8_t* scratch) const = 0;
...
extern Status ReadFileToString(Env* env, const std::string& fname, faststring* data);
...
class EnvWrapper : public Env {
 public:
  ...
  Status NewSequentialFile(
      const std::string& f, gscoped_ptr<SequentialFile>* r) OVERRIDE {
    return target_->NewSequentialFile(f, r);
  }
  Status NewRandomAccessFile(
      const std::string& f, gscoped_ptr<RandomAccessFile>* r) OVERRIDE {
    return target_->NewRandomAccessFile(f, r);
  }
  Status NewRandomAccessFile(const RandomAccessFileOptions& opts, const std::string& f,
  ...
  Status NewWritableFile(const WritableFileOptions& o, const std::string& f,
  ...
  Status NewRWFile(
      const RWFileOptions& o, const std::string& f, gscoped_ptr<RWFile>* r) OVERRIDE {
    return target_->NewRWFile(o, f, r);
  }
  Status NewTempRWFile(const RWFileOptions& o, const std::string& t, std::string* f,
  ...
  Status GetFileSizeOnDiskRecursively(
      const std::string& root, uint64_t* bytes_used) OVERRIDE {
  ...
  uint64_t gettid() OVERRIDE { return target_->gettid(); }
  ...
  Status Walk(
      const std::string& root, DirectoryOrder order, const WalkCallback& cb) OVERRIDE {
