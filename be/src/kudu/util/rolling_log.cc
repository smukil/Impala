...
string RollingLog::GetLogFileName(int sequence) const {
  ...
  str << 1900 + tm_time.tm_year << setw(2) << 1 + tm_time.tm_mon << setw(2)
      << tm_time.tm_mday << '-' << setw(2) << tm_time.tm_hour << setw(2) << tm_time.tm_min
...
Status RollingLog::CompressFile(const std::string& path) const {
  ...
  RETURN_NOT_OK_PREPEND(
      env_->NewSequentialFile(path, &in_file), "Unable to open input file to compress");
  ...
  while (true) {
    Slice result;
    RETURN_NOT_OK_PREPEND(
        in_file->Read(arraysize(buf), &result, buf), "Unable to read from gzip input");
    if (result.size() == 0) {
      break;
    }
    ...
    if (n == 0) {
      int errnum;
      return Status::IOError("Unable to write to gzip output", gzerror(gzf, &errnum));
    }
  }
  closer.Cancel();
  ...
  WARN_NOT_OK(
      env_->DeleteFile(path), "Unable to delete gzip input file after compression");
  return Status::OK();
}
