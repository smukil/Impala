...
Status TimedFailureDetector::Track(
    const string& name, const MonoTime& now, const FailureDetectedCallback& callback) {
...
FailureDetector::NodeStatus TimedFailureDetector::GetNodeStatusUnlocked(
    const std::string& name, const MonoTime& now) {
...
void TimedFailureDetector::CheckForFailures(const MonoTime& now) {
  ...
  for (const CallbackMap::value_type& entry : callbacks) {
    ...
    callback.Run(
        node_name, Status::RemoteError(Substitute("Node '$0' failed", node_name)));
  }
}

RandomizedFailureMonitor::RandomizedFailureMonitor(
    uint32_t random_seed, int64_t period_mean_millis, int64_t period_stddev_millis)
...
Status RandomizedFailureMonitor::Start() {
  ...
  return Thread::Create("failure-monitors", "failure-monitor",
      &RandomizedFailureMonitor::RunThread, this, &thread_);
}
...
Status RandomizedFailureMonitor::MonitorFailureDetector(
    const string& name, const scoped_refptr<FailureDetector>& fd) {
  std::lock_guard<simple_spinlock> l(lock_);
  bool inserted = InsertIfNotPresent(&fds_, name, fd);
  if (PREDICT_FALSE(!inserted)) {
    return Status::AlreadyPresent(
        Substitute("Already monitoring failure detector '$0'", name));
  }
  return Status::OK();
}
