...
bool BitmapFindFirst(
    const uint8_t* bitmap, size_t offset, size_t bitmap_size, bool value, size_t* idx);

// Find the first set bit in the bitmap, at the specified offset.
inline bool BitmapFindFirstSet(
    const uint8_t* bitmap, size_t offset, size_t bitmap_size, size_t* idx) {
...
class TrueBitIterator {
 public:
  ...
  bool done() const { return cur_byte_idx_ >= n_bytes_; }
 ...
 private:
  void AdvanceToNextOneBit() {
    ...
    DVLOG(2) << "Found next nonzero byte at " << cur_byte_idx_ << " val=" << cur_byte_;
