...
class Random {
 ...
 public:
  explicit Random(uint32_t s) { Reset(s); }
  ...
  //
  // In the case that 'c' has fewer than 'k' elements then all elements in 'c' will be
  // selected.
  //
  ...
  void ReservoirSample(
      const Collection& c, int k, const Set& avoid, std::vector<T>* result) {
    result->clear();
    result->reserve(k);
    int i = 0;
    for (const T& elem : c) {
...
class StdUniformRNG {
 public:
  ...
  uint32_t operator()() { return r_->Next32(); }
