...
class FailureDetectorTest : public KuduTest {
 public:
  FailureDetectorTest()
    : KuduTest(),
      latch_(1),
      monitor_(new RandomizedFailureMonitor(
          SeedRandom(), kFailureMonitorMeanMillis, kFailureMonitorStddevMillis)) {}
 ...
 protected:
  void WaitForFailure() { latch_.Wait(); }
...
// that node everything is ok and that once we stop doing so the failure detection
// function
// gets called.
TEST_F(FailureDetectorTest, TestDetectsFailure) {
  ASSERT_OK(monitor_->Start());

  scoped_refptr<FailureDetector> detector(
      new TimedFailureDetector(MonoDelta::FromMilliseconds(
          kExpectedHeartbeatPeriodMillis * kMaxMissedHeartbeats)));

  monitor_->MonitorFailureDetector(kTestTabletName, detector);
  ASSERT_FALSE(detector->IsTracking(kNodeName));
  ASSERT_OK(detector->Track(kNodeName, MonoTime::Now(),
  ...
  const int kUpdatesPerPeriod =
      10; // Num updates we give per period to minimize test flakiness.
  ...
  for (int i = 0; i < kNumPeriodsToWait * kUpdatesPerPeriod; i++) {
    ...
    SleepFor(
        MonoDelta::FromMilliseconds(kExpectedHeartbeatPeriodMillis / kUpdatesPerPeriod));
