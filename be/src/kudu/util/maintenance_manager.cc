...
MaintenanceOp::MaintenanceOp(std::string name, IOUsage io_usage)
  : name_(std::move(name)), running_(0), cancel_(false), io_usage_(io_usage) {}
...
const MaintenanceManager::Options MaintenanceManager::DEFAULT_OPTIONS = {
    0, 0, 0, shared_ptr<MemTracker>(),
};

MaintenanceManager::MaintenanceManager(const Options& options)
  : num_threads_(options.num_threads <= 0 ? FLAGS_maintenance_manager_num_threads :
                                            options.num_threads),
    cond_(&lock_),
    shutdown_(false),
    running_ops_(0),
    ...
    parent_mem_tracker_(!options.parent_mem_tracker ? MemTracker::GetRootTracker() :
                                                      options.parent_mem_tracker) {
...
Status MaintenanceManager::Init() {
  RETURN_NOT_OK(Thread::Create("maintenance", "maintenance_scheduler",
      boost::bind(&MaintenanceManager::RunSchedulerThread, this), &monitor_thread_));
  return Status::OK();
}
...
void MaintenanceManager::UnregisterOp(MaintenanceOp* op) {
  {
    std::lock_guard<Mutex> guard(lock_);
    CHECK(op->manager_.get() == this)
        << "Tried to unregister " << op->name()
...
// - If we've hit the overall process memory limit (note: this includes memory that the
// Ops cannot
...
// that has
//   the highest retention (and if many qualify, then we run the one that also frees up
//   the
//   most RAM).
...
//
// The reason it's done this way is that we want to prioritize limiting the amount of
// resources we
// hold on to. Low IO Ops go first since we can quickly run them, then we can look at
// memory usage.
// Reversing those can starve the low IO Ops when the system is under intense memory
// pressure.
//
// In the third priority we're at a point where nothing's urgent and there's nothing we
// can run
// quickly.
...
MaintenanceOp* MaintenanceManager::FindBestOp() {
  ...
  if (low_io_most_logs_retained_bytes_op) {
    if (low_io_most_logs_retained_bytes > 0) {
      VLOG_AND_TRACE("maintenance", 1)
          ...
          << "at " << low_io_most_logs_retained_bytes << " bytes with a low IO cost";
  ...
  if (parent_mem_tracker_->AnySoftLimitExceeded(&capacity_pct)) {
    if (!most_mem_anchored_op) {
      string msg = StringPrintf(
          "we have exceeded our soft memory limit "
  ...
  if (most_logs_retained_bytes_op
    ...
    VLOG_AND_TRACE("maintenance", 1)
        ...
        << "because it can free up more logs (" << most_logs_retained_bytes << " bytes)";
  ...
  if (best_perf_improvement_op) {
    if (best_perf_improvement > 0) {
      VLOG_AND_TRACE("maintenance", 1)
          << "Performing " << best_perf_improvement_op->name() << ", "
...
void MaintenanceManager::LaunchOp(MaintenanceOp* op) {
  ...
  LOG_TIMING(INFO, Substitute("running $0", op->name())) {
    ...
    TRACE_EVENT1("maintenance", "MaintenanceManager::LaunchOp", "name", op->name());
    op->Perform();
  }
