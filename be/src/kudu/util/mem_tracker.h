...
// tally maintained by Consume() and Release(). A tcmalloc function is used to track
// process
...
//    e.g., a tablet server's memtracker will have as its children the tablets'
//    memtrackers,
...
class MemTracker : public std::enable_shared_from_this<MemTracker> {
 public:
  ...
  static std::shared_ptr<MemTracker> CreateTracker(int64_t byte_limit,
      const std::string& id,
  ...
  // of 'parent'.
  static bool FindTracker(const std::string& id, std::shared_ptr<MemTracker>* tracker,
      const std::shared_ptr<MemTracker>& parent = std::shared_ptr<MemTracker>());
 ...
 private:
  ...
  static std::string LogUsage(
      const std::string& prefix, const std::list<std::weak_ptr<MemTracker>>& trackers);

  // Variant of FindTracker() that must be called with a non-NULL parent.
  static bool FindTrackerInternal(const std::string& id,
      std::shared_ptr<MemTracker>* tracker, const std::shared_ptr<MemTracker>& parent);
...
class ScopedTrackedConsumption {
 public:
  ScopedTrackedConsumption(std::shared_ptr<MemTracker> tracker, int64_t to_consume)
  ...
  ~ScopedTrackedConsumption() { tracker_->Release(consumption_); }
