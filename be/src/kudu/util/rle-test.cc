...
template <typename T>
void ValidateRle(const vector<T>& values, int bit_width, uint8_t* expected_encoding,
    int expected_len) {
  faststring buffer;
...
static size_t GenerateRandomBitString(
    int num_runs, faststring* enc_buf, string* string_rep) {
...
TEST_F(TestRle, TestRoundTripRandomSequencesWithRuns) {
  ...
  for (int rep = 0; rep < 100; rep++) {
    ...
    bool val;
    while (rem_to_read > 0
        && (run_len = decoder.GetNextRun(&val, std::min(kMaxToReadAtOnce, rem_to_read)))
            != 0) {
