...
template <class Traits>
void IntervalTree<Traits>::FindContainingPoint(
    const point_type& query, IntervalVector* results) const {
  if (root_) {
...
template <class Traits>
void IntervalTree<Traits>::FindIntersectingInterval(
    const interval_type& query, IntervalVector* results) const {
  if (root_) {
...
static bool LessThan(
    const typename Traits::point_type& a, const typename Traits::point_type& b) {
...
void IntervalTree<Traits>::Partition(const IntervalVector& in, point_type* split_point,
    IntervalVector* left, IntervalVector* overlapping, IntervalVector* right) {
  CHECK(!in.empty());
...
class ITNode {
 ...
 public:
  ...
  void FindContainingPoint(const point_type& query, IntervalVector* results) const;
...
ITNode<Traits>::ITNode(typename Traits::point_type split_point, ITNode<Traits>* left,
  ...
  std::sort(
      overlapping_by_asc_left_.begin(), overlapping_by_asc_left_.end(), SortByAscLeft);
...
void ITNode<Traits>::FindContainingPoint(
  ...
  if (cmp < 0) {
    ...
    results->insert(
        results->end(), overlapping_by_asc_left_.begin(), overlapping_by_asc_left_.end());
  }
}
