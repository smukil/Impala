...
BloomFilterSizing BloomFilterSizing::ByCountAndFPRate(
  ...
  double n_bits =
      -static_cast<double>(expected_count) * log(fp_rate) / kNaturalLog2 / kNaturalLog2;
  int n_bytes = static_cast<int>(ceil(n_bits / 8));
  CHECK_GT(n_bytes, 0) << "expected_count: " << expected_count << " fp_rate: " << fp_rate;
...
double BloomFilterBuilder::false_positive_rate() const {
  ...
  return pow(
      1 - exp(-static_cast<double>(n_hashes_) * expected_count_ / n_bits_), n_hashes_);
}
