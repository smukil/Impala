...
class AtomicInt {
 public:
  ...
  // Does not support 'kMemOrderBarrier'.
  bool CompareAndSet(
      T expected_val, T new_value, MemoryOrder mem_order = kMemOrderNoBarrier);
 ...
 private:
  ...
  static void FatalMemOrderNotSupported(const char* caller,
      ...
      const char* supported = "kMemNorderNoBarrier, kMemOrderAcquire, kMemOrderRelease");
...
class AtomicBool {
 public:
  ...
  bool Load(MemoryOrder m = kMemOrderNoBarrier) const { return underlying_.Load(m); }
...
template <typename T>
inline bool AtomicInt<T>::CompareAndSet(
    T expected_val, T new_val, MemoryOrder mem_order) {
...
inline T AtomicInt<T>::CompareAndSwap(T expected_val, T new_val, MemoryOrder mem_order) {
  switch (mem_order) {
    case kMemOrderNoBarrier: {
      return base::subtle::NoBarrier_CompareAndSwap(&value_, expected_val, new_val);
    }
    case kMemOrderBarrier: {
    ...
    case kMemOrderAcquire: {
      return base::subtle::Acquire_CompareAndSwap(&value_, expected_val, new_val);
    }
    case kMemOrderRelease: {
      return base::subtle::Release_CompareAndSwap(&value_, expected_val, new_val);
    }
  }
  abort();
}
...
inline T AtomicInt<T>::IncrementBy(T delta, MemoryOrder mem_order) {
  switch (mem_order) {
    ...
    case kMemOrderAcquire: {
      FatalMemOrderNotSupported("Increment/IncrementBy", "kMemOrderAcquire",
