...
class CowObject {
 public:
  ...
  void ReadLock() const { lock_.ReadLock(); }
  ...
  const State& dirty() const { return *DCHECK_NOTNULL(dirty_state_.get()); }
...
class CowLock {
 public:
  enum LockMode { READ, WRITE, RELEASED };
  ...
  bool is_write_locked() const { return mode_ == WRITE; }
