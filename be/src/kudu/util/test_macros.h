...
#define ASSERT_STR_CONTAINS(str, substr)                                                \
  do {                                                                                  \
    ...
    if (_s.find((substr)) == std::string::npos) {                                       \
      FAIL() << "Expected to find substring '" << (substr) << "'. Got: '" << _s << "'"; \
    }                                                                                   \
  } while (0);
...
#define ASSERT_STR_NOT_CONTAINS(str, substr)                                        \
  do {                                                                              \
    const std::string& _s = (str);                                                  \
    if (_s.find((substr)) != std::string::npos) {                                   \
      FAIL() << "Expected not to find substring '" << (substr) << "'. Got: '" << _s \
             << "'";                                                                \
    }                                                                               \
  } while (0);

// Substring regular expressions in extended regex (POSIX) syntax.
#define ASSERT_STR_MATCHES(str, pattern) ASSERT_THAT(str, testing::ContainsRegex(pattern))
...
#define ASSERT_STRINGS_ALL_MATCH(strings, pattern)                                       \
  do {                                                                                   \
    ...
    for (const auto& str : _strings) {                                                   \
      ASSERT_STR_MATCHES(str, _pattern) << "string " << _str_idx << ": pattern "         \
                                        << _pattern << " does not match string " << str; \
      _str_idx++;                                                                        \
    }                                                                                    \
  } while (0)
