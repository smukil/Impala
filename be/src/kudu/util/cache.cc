...
struct LRUHandle {
  ...
  Slice key() const { return Slice(kv_data, key_length); }
  ...
  Slice value() const { return Slice(val_ptr(), val_length); }
};
...
LRUCache::LRUCache(MemTracker* tracker)
  : usage_(0), mem_tracker_(tracker), metrics_(nullptr) {
...
Cache::Handle* LRUCache::Insert(
    LRUHandle* e, Cache::EvictionCallback* eviction_callback) {
...
class ShardedLRUCache : public Cache {
 ...
 public:
  explicit ShardedLRUCache(size_t capacity, const string& id)
    : last_id_(0), shard_bits_(DetermineShardBits()) {
  ...
  virtual ~ShardedLRUCache() { STLDeleteElements(&shards_); }

  virtual Handle* Insert(
      PendingHandle* handle, Cache::EvictionCallback* eviction_callback) OVERRIDE {
  ...
  virtual PendingHandle* Allocate(Slice key, int val_len, int charge) OVERRIDE {
    ...
    uint8_t* buf =
        new uint8_t[sizeof(LRUHandle) + key_len_padded + val_len // the kv_data VLA data
