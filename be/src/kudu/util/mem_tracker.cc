...
static bool dummy[] = {google::RegisterFlagValidator(
                           &FLAGS_memory_limit_soft_percentage, &ValidatePercentage),
    google::RegisterFlagValidator(&FLAGS_memory_limit_warn_threshold_percentage,
                           &ValidatePercentage)
#ifdef TCMALLOC_ENABLED
        ,
    google::RegisterFlagValidator(
                           &FLAGS_tcmalloc_max_free_bytes_percentage, &ValidatePercentage)
#endif
};
...
void MemTracker::CreateRootTracker() {
...
#endif
  root_tracker.reset(new MemTracker(f, limit, "root", shared_ptr<MemTracker>()));
  root_tracker->Init();
  ...
  LOG(INFO) << StringPrintf("MemTracker: soft memory limit is %.6f GB",
      (static_cast<float>(root_tracker->soft_limit_) / (1024.0 * 1024.0 * 1024.0)));
}

shared_ptr<MemTracker> MemTracker::CreateTracker(
    int64_t byte_limit, const string& id, const shared_ptr<MemTracker>& parent) {
...
bool MemTracker::FindTrackerInternal(const string& id, shared_ptr<MemTracker>* tracker,
    const shared_ptr<MemTracker>& parent) {
  DCHECK(parent != NULL);
  ...
  if (PREDICT_TRUE(found.size() == 1)) {
    *tracker = found[0];
    ...
    LOG(DFATAL) << Substitute("Multiple memtrackers with same id ($0) found on parent $1",
        id, parent->ToString());
    *tracker = found[0];
    return true;
  }
  ...
  for (i = all_trackers_.size() - 1; i >= 0; --i) {
    ...
    if (tracker->limit_ < 0) {
      ...
      if (!tracker->consumption_.TryIncrementBy(bytes, tracker->limit_)) {
        ...
        if (!tracker->GcMemory(tracker->limit_ - bytes) || tracker->ExpandLimit(bytes)) {
          if (!tracker->consumption_.TryIncrementBy(bytes, tracker->limit_)) {
            break;
          }
        } else {
...
void MemTracker::Release(int64_t bytes) {
  ...
  if (PREDICT_FALSE(
          base::subtle::Barrier_AtomicIncrement(&released_memory_since_gc, bytes)
          > GC_RELEASE_SIZE)) {
    GcTcmalloc();
  }
...
bool MemTracker::SoftLimitExceeded(double* current_capacity_pct) {
  ...
  if (LimitExceeded()) {
    if (current_capacity_pct) {
      *current_capacity_pct = static_cast<double>(consumption()) / limit() * 100;
    }
    return true;
  }
...
string MemTracker::LogUsage(
    const string& prefix, const list<weak_ptr<MemTracker>>& trackers) {
