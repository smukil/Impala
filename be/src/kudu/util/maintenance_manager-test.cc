...
METRIC_DEFINE_gauge_uint32(test, maintenance_ops_running,
    "Number of Maintenance Operations Running", kudu::MetricUnit::kMaintenanceOperations,
    "The number of background maintenance operations currently running.");
METRIC_DEFINE_histogram(test, maintenance_op_duration, "Maintenance Operation Duration",
...
class MaintenanceManagerTest : public KuduTest {
 public:
  ...
  ~MaintenanceManagerTest() { manager_->Shutdown(); }
...
class TestMaintenanceOp : public MaintenanceOp {
 public:
  TestMaintenanceOp(
      const std::string& name, IOUsage io_usage, const shared_ptr<MemTracker>& tracker)
...
TEST_F(MaintenanceManagerTest, TestRegisterUnregister) {
  ...
  AssertEventually([&]() { ASSERT_EQ(op1.DurationHistogram()->TotalCount(), 1); });
...
TEST_F(MaintenanceManagerTest, TestNewOpsDontGetScheduledDuringUnregister) {
  ...
  AssertEventually([&]() { ASSERT_EQ(op1.RunningGauge()->value(), 2); });
...
TEST_F(MaintenanceManagerTest, TestMemoryPressure) {
  ...
  AssertEventually([&]() { ASSERT_EQ(op.DurationHistogram()->TotalCount(), 1); });
...
TEST_F(MaintenanceManagerTest, TestLogRetentionPrioritization) {
  ...
  // Low IO is taken care of, now we find the op that clears the most log retention and
  // ram.
