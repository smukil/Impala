...
string VersionInfo::GetShortVersionString() {
  return strings::Substitute("kudu $0 (rev $1)", KUDU_VERSION_STRING, GetGitHash());
}

string VersionInfo::GetAllVersionInfo() {
  string ret = strings::Substitute("kudu $0\n"
                                   "revision $1\n"
                                   "build type $2\n"
                                   "built by $3 at $4 on $5",
      KUDU_VERSION_STRING, GetGitHash(), KUDU_BUILD_TYPE, KUDU_BUILD_USERNAME,
      KUDU_BUILD_TIMESTAMP, KUDU_BUILD_HOSTNAME);
