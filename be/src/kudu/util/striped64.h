...
class Cell {
 public:
  ...
  char pad[CACHELINE_SIZE > ATOMIC_INT_SIZE ? CACHELINE_SIZE - ATOMIC_INT_SIZE : 1];
...
//
// The Striped64 and LongAdder implementations here are simplified versions of what's
// present in
...
//
// These two problems are addressed by Striped64. When there is no contention, it uses CAS
// on a
...
//
// Reading the value of a Striped64 requires traversing the hashtable to calculate the
// true sum.
//
// Each updating thread uses a thread-local hashcode to determine its Cell in the
// hashtable.
// If a thread fails to CAS its hashed Cell, it will do a lightweight rehash operation to
// try
// and find an uncontended bucket. Because the hashcode is thread-local, this rehash
// affects all
...
//
// The hashtable is statically sized to the nearest power of 2 greater than or equal to
// the
// number of CPUs. This is sufficient, since this guarantees the existence of a perfect
// hash
// function. Due to the random rehashing, the threads should eventually converge to this
// function.
...
// This means we waste some additional memory in low contention scenarios, and initial
// allocation
...
class Striped64 {
 ...
 protected:
  ...
  // Static hash code per-thread. Shared across all instances to limit thread-local
  // pollution.
