...
TEST_F(SubprocessTest, TestReadFromStdoutAndStderr) {
  ...
  ASSERT_OK(Subprocess::Call(
      {"/bin/bash", "-c", "dd if=/dev/urandom of=/dev/stdout bs=512 count=2048 &"
...
TEST_F(SubprocessTest, TestCallWithStdin) {
  string stdout;
  ASSERT_OK(Subprocess::Call({"/bin/bash"}, "echo \"quick brown fox\"", &stdout));
...
TEST_F(SubprocessTest, TestGetExitStatusExitFailure) {
  ...
  for (auto code : kStatusCodes) {
    ...
    ASSERT_STR_CONTAINS(
        exit_info, Substitute("process exited with non-zero status $0", exit_status));
  }
}

TEST_F(SubprocessTest, TestGetExitStatusSignaled) {
  static const vector<int> kSignals = {
      SIGHUP, SIGABRT, SIGKILL, SIGTERM, SIGUSR1,
  };
  for (auto signum : kSignals) {
    ...
    ASSERT_STR_CONTAINS(exit_info, Substitute("process exited on signal $0", signum));
  }
}
