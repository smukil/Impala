...
#define LOG_TIMING_PREFIX_IF(severity, condition, prefix, description)                  \
  for (kudu::sw_internal::LogTiming _l(                                                 \
           __FILE__, __LINE__, google::severity, prefix, description, -1, (condition)); \
...
#define LOG_TIMING(severity, description) LOG_TIMING_IF(severity, true, (description))
...
#define LOG_SLOW_EXECUTION(severity, max_expected_millis, description)           \
  for (kudu::sw_internal::LogTiming _l(__FILE__, __LINE__, google::severity, "", \
           description, max_expected_millis, true);                              \
       !_l.HasRun(); _l.MarkHasRun())
...
#define VLOG_TIMING(vlog_level, description)                                 \
  for (kudu::sw_internal::LogTiming _l(__FILE__, __LINE__, google::INFO, "", \
           description, -1, VLOG_IS_ON(vlog_level));                         \
       !_l.HasRun(); _l.MarkHasRun())

// Macro to log the time spent in the rest of the block.
#define SCOPED_VLOG_TIMING(vlog_level, description)          \
  kudu::sw_internal::LogTiming VARNAME_LINENUM(_log_timing)( \
      __FILE__, __LINE__, google::INFO, "", description, -1, VLOG_IS_ON(vlog_level));
...
struct CpuTimes {
  ...
  std::string ToString() const {
    return StringPrintf("real %.3fs\tuser %.3fs\tsys %.3fs", wall_seconds(),
        user_cpu_seconds(), system_cpu_seconds());
  }

  double wall_millis() const { return static_cast<double>(wall) / NANOS_PER_MILLISECOND; }

  double wall_seconds() const { return static_cast<double>(wall) / NANOS_PER_SECOND; }

  double user_cpu_seconds() const { return static_cast<double>(user) / NANOS_PER_SECOND; }
...
class Stopwatch {
 public:
  ...
  explicit Stopwatch(Mode mode = THIS_THREAD) : stopped_(true), mode_(mode) {
    times_.clear();
  }
...
class LogTiming {
 public:
  LogTiming(const char* file, int line, google::LogSeverity severity, std::string prefix,
      std::string description, int64_t max_expected_millis, bool should_print)
    : file_(file),
      line_(line),
      severity_(severity),
  ...
  void Print(int64_t max_expected_millis) {
    ...
    if (times.wall_millis() > max_expected_millis) {
      google::LogMessage(file_, line_, severity_).stream()
          << prefix_ << "Time spent " << description_ << ": " << times.ToString();
    }
  }
};
