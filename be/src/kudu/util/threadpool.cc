...
ThreadPoolBuilder& ThreadPoolBuilder::set_trace_metric_prefix(const std::string& prefix) {
...
ThreadPool::ThreadPool(const ThreadPoolBuilder& builder)
  : name_(builder.name_),
    ...
    active_threads_(0),
    queue_size_(0) {
  string prefix = !builder.trace_metric_prefix_.empty() ? builder.trace_metric_prefix_ :
                                                          builder.name_;

  queue_time_trace_metric_name_ = TraceMetrics::InternName(prefix + ".queue_time_us");
  run_wall_time_trace_metric_name_ =
      TraceMetrics::InternName(prefix + ".run_wall_time_us");
  run_cpu_time_trace_metric_name_ = TraceMetrics::InternName(prefix + ".run_cpu_time_us");
}
...
Status ThreadPool::Submit(const std::shared_ptr<Runnable>& task) {
  ...
  if (queue_size_ == max_queue_size_) {
    return Status::ServiceUnavailable(
        Substitute("Thread pool queue is full ($0 items)", queue_size_));
  }
  ...
  if (additional_threads > 0 && num_threads_ < max_threads_) {
    ...
    if (!status.ok()) {
      if (num_threads_ == 0) {
        ...
        LOG(WARNING) << "Thread pool failed to create thread: " << status.ToString();
      }
    }
  }
...
Status ThreadPool::CreateThreadUnlocked() {
  ...
  Status s =
      kudu::Thread::Create("thread pool", strings::Substitute("$0 [worker]", name_),
