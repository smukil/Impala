...
class ArenaBase {
 public:
  ...
  ArenaBase(BufferAllocator* const buffer_allocator, size_t initial_buffer_size,
  ...
  // If this request would make the arena grow and the allocator denies that,
  // returns NULL and leaves the arena unchanged.
  void* AllocateBytes(const size_t size) { return AllocateBytesAligned(size, 1); }
  ...
  inline void ReleaseStoreCurrent(Component* c) {
    if (THREADSAFE) {
      base::subtle::Release_Store(
          reinterpret_cast<AtomicWord*>(&current_), reinterpret_cast<AtomicWord>(c));
    } else {
      current_ = c;
    }
  }
...
class ArenaAllocator {
 public:
  ...
  ArenaBase<THREADSAFE>* arena() const { return arena_; }
...
class MemoryTrackingArena : public ArenaBase<false> {
 public:
  MemoryTrackingArena(size_t initial_buffer_size, size_t max_buffer_size,
...
class ThreadSafeMemoryTrackingArena : public ArenaBase<true> {
 public:
  ThreadSafeMemoryTrackingArena(size_t initial_buffer_size, size_t max_buffer_size,
      const std::shared_ptr<MemoryTrackingBufferAllocator>& tracking_allocator)
...
class ArenaBase<THREADSAFE>::Component {
 public:
  ...
  uint8_t* AllocateBytes(const size_t size) { return AllocateBytesAligned(size, 1); }
...
inline bool ArenaBase<THREADSAFE>::RelocateStringPiece(
    const StringPiece& src, StringPiece* sp) {
