...
class BufferAllocator {
 public:
  ...
  Buffer* Allocate(size_t requested) { return BestEffortAllocate(requested, requested); }
  ...
  // 2. If minimal == 0, the allocator will always return a non-NULL Buffer
  //    with a non-NULL data pointer, possibly with zero capacity.
  Buffer* BestEffortReallocate(size_t requested, size_t minimal, Buffer* buffer) {
    DCHECK_LE(minimal, requested);
    Buffer* result;
    if (buffer == NULL) {
      ...
      result = ReallocateInternal(requested, minimal, buffer, this) ? buffer : NULL;
 ...
 protected:
  ...
  Buffer* DelegateAllocate(BufferAllocator* delegate, size_t requested, size_t minimal,
      BufferAllocator* originator) {
    return delegate->AllocateInternal(requested, minimal, originator);
  }

  // Called by chained buffer allocators.
  bool DelegateReallocate(BufferAllocator* delegate, size_t requested, size_t minimal,
      Buffer* buffer, BufferAllocator* originator) {
 ...
 private:
  ...
  virtual Buffer* AllocateInternal(
      size_t requested, size_t minimal, BufferAllocator* originator) = 0;

  // Implemented by concrete subclasses. Returns false on failure.
  virtual bool ReallocateInternal(
      size_t requested, size_t minimal, Buffer* buffer, BufferAllocator* originator) = 0;
...
class HeapBufferAllocator : public BufferAllocator {
 public:
  ...
  static HeapBufferAllocator* Get() { return Singleton<HeapBufferAllocator>::get(); }

  virtual size_t Available() const OVERRIDE { return numeric_limits<size_t>::max(); }
 ...
 private:
  ...
  virtual Buffer* AllocateInternal(
      size_t requested, size_t minimal, BufferAllocator* originator) OVERRIDE;

  virtual bool ReallocateInternal(size_t requested, size_t minimal, Buffer* buffer,
  ...
  HeapBufferAllocator();
  explicit HeapBufferAllocator(bool aligned_mode) : aligned_mode_(aligned_mode) {}
...
class ClearingBufferAllocator : public BufferAllocator {
 public:
  ...
  explicit ClearingBufferAllocator(BufferAllocator* delegate) : delegate_(delegate) {}

  virtual size_t Available() const OVERRIDE { return delegate_->Available(); }
...
class StaticQuota : public Quota<thread_safe> {
 public:
  explicit StaticQuota(size_t quota) : Quota<thread_safe>(true) { SetQuota(quota); }
...
class MediatingBufferAllocator : public BufferAllocator {
 public:
  ...
  // both to be reused.
  MediatingBufferAllocator(BufferAllocator* const delegate, Mediator* const mediator)
...
class MemoryLimit : public BufferAllocator {
 public:
  ...
  virtual size_t Available() const OVERRIDE { return allocator_.Available(); }
...
class SoftQuotaBypassingBufferAllocator : public BufferAllocator {
 public:
  SoftQuotaBypassingBufferAllocator(BufferAllocator* allocator, size_t bypassed_amount)
 ...
 private:
  ...
  virtual bool ReallocateInternal(size_t requested, size_t minimal, Buffer* buffer,
      BufferAllocator* originator) OVERRIDE {
    if (DelegateReallocate(&allocator_, requested, AdjustMinimal(requested, minimal),
            buffer, originator)) {
      return true;
    } else {
      return DelegateReallocate(&allocator_, requested, minimal, buffer, originator);
...
class MemoryStatisticsCollectingBufferAllocator : public BufferAllocator {
 public:
  ...
  MemoryStatisticsCollectingBufferAllocator(BufferAllocator* const delegate,
...
class MemoryTrackingBufferAllocator : public BufferAllocator {
 public:
  ...
  MemoryTrackingBufferAllocator(BufferAllocator* const delegate,
      std::shared_ptr<MemTracker> mem_tracker, bool enforce_limit = false)
    : delegate_(delegate),
...
class ThreadSafeMemoryLimit : public OwningThreadSafeBufferAllocator<MemoryLimit> {
 public:
  ThreadSafeMemoryLimit(size_t quota, bool enforced, BufferAllocator* const delegate)
...
class OwningBufferAllocator : public BufferAllocator {
 public:
  ...
  explicit OwningBufferAllocator(BufferAllocator* const delegate) : delegate_(delegate) {}
 ...
 private:
  ...
  virtual void FreeInternal(Buffer* buffer) OVERRIDE { DelegateFree(delegate_, buffer); }
...
class GuaranteeMemory : public BufferAllocator {
 ...
 private:
  ...
  virtual void FreeInternal(Buffer* buffer) OVERRIDE { DelegateFree(&limit_, buffer); }
...
template <bool thread_safe>
size_t Quota<thread_safe>::Allocate(const size_t requested, const size_t minimal) {
  lock_guard_maybe<Mutex> lock(mutex());
  DCHECK_LE(minimal, requested) << "\"minimal\" shouldn't be bigger than \"requested\"";
  const size_t quota = GetQuotaInternal();
  size_t allocation;
