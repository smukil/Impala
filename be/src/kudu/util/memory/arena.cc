...
template <bool THREADSAFE>
ArenaBase<THREADSAFE>::ArenaBase(BufferAllocator* const buffer_allocator,
    size_t initial_buffer_size, size_t max_buffer_size)
...
typename ArenaBase<THREADSAFE>::Component* ArenaBase<THREADSAFE>::NewComponent(
    size_t requested_size, size_t minimum_size) {
...
void ArenaBase<THREADSAFE>::AddComponent(ArenaBase::Component* component) {
  ...
  if (PREDICT_FALSE(arena_footprint_ > FLAGS_arena_warn_threshold_bytes) && !warned_) {
    LOG(WARNING) << "Arena " << reinterpret_cast<const void*>(this) << " footprint ("
                 << arena_footprint_ << " bytes) exceeded warning threshold ("
