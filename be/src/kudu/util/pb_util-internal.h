...
class SequentialFileFileInputStream : public google::protobuf::io::ZeroCopyInputStream {
 public:
  explicit SequentialFileFileInputStream(
      SequentialFile* rfile, size_t buffer_size = kDefaultBufferSize)
    : buffer_used_(0),
      buffer_offset_(0),
  ...
  int64 ByteCount() const OVERRIDE { return total_read_; }
...
class WritableFileOutputStream : public google::protobuf::io::ZeroCopyOutputStream {
 public:
  explicit WritableFileOutputStream(
      WritableFile* wfile, size_t buffer_size = kDefaultBufferSize)
    : buffer_offset_(0),
      buffer_size_(buffer_size),
      buffer_(new uint8[buffer_size_]),
      flushed_(0),
  ...
  int64 ByteCount() const OVERRIDE { return flushed_ + buffer_offset_; }
