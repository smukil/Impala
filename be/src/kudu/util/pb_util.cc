...
void ByteSizeConsistencyError(int byte_size_before_serialization,
    int byte_size_after_serialization, int bytes_produced_by_serialization) {
...
template <typename T>
Status NonShortRead(
    T* file, uint64_t offset, uint64_t length, Slice* result, uint8_t* scratch);

template <>
Status NonShortRead<RandomAccessFile>(RandomAccessFile* file, uint64_t offset,
    uint64_t length, Slice* result, uint8_t* scratch) {
  return env_util::ReadFully(file, offset, length, result, scratch);
}

template <>
Status NonShortRead<RWFile>(
    RWFile* file, uint64_t offset, uint64_t length, Slice* result, uint8_t* scratch) {
...
Status ValidateAndReadData(ReadableFileType* reader, uint64_t file_size, uint64_t* offset,
    uint64_t length, Slice* result, gscoped_ptr<uint8_t[]>* scratch) {
    ...
    return Status::Incomplete("File size not large enough to be valid",
        Substitute("Proto container file $0: "
                   ...
                   "$2 but file size is only $3 bytes",
                                  reader->filename(), length, *offset, file_size));
  }
  ...
  CHECK_EQ(length, s.size()) // Should never trigger due to contract with reader APIs.
      << Substitute(
          "Unexpected short read: Proto container file $0: Tried to read $1 bytes "
...
#define CHECKSUM_ERR_MSG(prefix, filename, cksum_offset) \
  Substitute(                                            \
      "$0: Incorrect checksum in file $1 at offset $2", prefix, filename, cksum_offset)
...
Status ParseAndCompareChecksum(
    const uint8_t* checksum_buf, const initializer_list<Slice>& slices) {
  ...
  if (PREDICT_FALSE(actual_checksum != written_checksum)) {
    return Status::Corruption(
        Substitute("Checksum does not match. Expected: $0. Actual: $1", written_checksum,
            actual_checksum));
  }
  return Status::OK();
}
...
Status ReadPBStartingAt(
    ReadableFileType* reader, int version, uint64_t* offset, Message* msg) {
  ...
  uint64_t length_buflen =
      (version == 1) ? sizeof(uint32_t) : sizeof(uint32_t) + kPBContainerChecksumLen;
  ...
  if (version >= 2) {
    Slice length_checksum(
        len_and_cksum_slice.data() + sizeof(uint32_t), kPBContainerChecksumLen);
    RETURN_NOT_OK_PREPEND(ParseAndCompareChecksum(length_checksum.data(), {length}),
        CHECKSUM_ERR_MSG("Data length checksum does not match", reader->filename(),
  ...
  RETURN_NOT_OK_PREPEND(ValidateAndReadData(reader, file_size, &tmp_offset,
                            data_and_cksum_buflen, &body_and_cksum_slice, &body_scratch),
  ...
  Slice record_checksum(
      body_and_cksum_slice.data() + data_length, kPBContainerChecksumLen);
  ...
  if (version == 1) {
    RETURN_NOT_OK_PREPEND(ParseAndCompareChecksum(record_checksum.data(), {length, body}),
        CHECKSUM_ERR_MSG("Length and data checksum does not match", reader->filename(),
                              tmp_offset - kPBContainerChecksumLen));
  } else {
    RETURN_NOT_OK_PREPEND(ParseAndCompareChecksum(record_checksum.data(), {body}),
        CHECKSUM_ERR_MSG("Data checksum does not match", reader->filename(),
                              tmp_offset - kPBContainerChecksumLen));
  }
...
Status ParsePBFileHeader(ReadableFileType* reader, uint64_t* offset, int* version) {
  ...
  RETURN_NOT_OK_PREPEND(ValidateAndReadData(reader, file_size, &tmp_offset,
                            kPBContainerV2HeaderLen, &header, &scratch),
  ...
  Slice checksum(
      header.data() + kPBContainerMagicLen + sizeof(uint32_t), kPBContainerChecksumLen);

  // Validate magic number.
  if (PREDICT_FALSE(
          !strings::memeq(kPBContainerMagic, header.data(), kPBContainerMagicLen))) {
    ...
    return Status::Corruption("Invalid magic number",
        Substitute("Expected: $0, found: $1", Utf8SafeCEscape(kPBContainerMagic),
  ...
  if (tmp_version >= 2) {
    RETURN_NOT_OK_PREPEND(ParseAndCompareChecksum(checksum.data(), {magic_and_version}),
        CHECKSUM_ERR_MSG("File header checksum does not match", reader->filename(),
                              tmp_offset - kPBContainerChecksumLen));
  } else {
...
Status WritePBToPath(
    Env* env, const std::string& path, const MessageLite& msg, SyncMode sync) {
        ...
        switch (field->cpp_type()) {
          case FieldDescriptor::CPPTYPE_STRING: {
            const string& s_const =
                reflection->GetRepeatedStringReference(*message, field, i, nullptr);
...
Status WritablePBContainerFile::Init(const Message& msg) {
  ...
  ContainerSupHeaderPB sup_header;
  PopulateDescriptorSet(msg.GetDescriptor()->file(), sup_header.mutable_protos());
...
Status WritablePBContainerFile::Reopen() {
  ...
  RETURN_NOT_OK(
      writer_->Size(&offset_)); // Reset the write offset to the end of the file.
  state_ = FileState::OPEN;
  return Status::OK();
}
...
Status ReadablePBContainerFile::Dump(ostream* os, bool oneline) {
  ...
  if (!prototype) {
    return Status::NotSupported("Descriptor not supported",
        Substitute("Descriptor $0 referenced in container file not supported",
                                    pb_type()));
  }
...
Status WritePBContainerToPath(Env* env, const std::string& path, const Message& msg,
    CreateMode create, SyncMode sync) {
  TRACE_EVENT2(
      "io", "WritePBContainerToPath", "path", path, "msg_type", msg.GetTypeName());
