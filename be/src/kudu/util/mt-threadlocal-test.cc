...
class CounterRegistry {
 public:
  ...
  RegistryLockType* get_lock() const { return &lock_; }
  ...
  CounterPtrSet* GetCountersUnlocked() { return &counters_; }
...
class Counter {
 public:
  Counter(CounterRegistry* registry, int val)
    : tid_(Env::Default()->gettid()), registry_(CHECK_NOTNULL(registry)), val_(val) {
  ...
  uint64_t tid() { return tid_; }

  CounterLockType* get_lock() const { return &lock_; }

  void IncrementUnlocked() { val_++; }

  int GetValueUnlocked() { return val_; }
...
static void RegisterCounterAndLoopIncr(CounterRegistry* registry,
    CountDownLatch* counters_ready, CountDownLatch* reader_ready,
    CountDownLatch* counters_done, CountDownLatch* reader_done) {
...
static void RunAndAssign(CountDownLatch* writers_ready, CountDownLatch* readers_ready,
    CountDownLatch* all_done, CountDownLatch* threads_exiting, const std::string& in,
    std::string* out) {
  writers_ready->Wait();
...
TEST_F(ThreadLocalTest, TestTLSMember) {
  ...
  for (int i = 0; i < num_threads; i++) {
    ...
    scoped_refptr<kudu::Thread> new_thread;
    CHECK_OK(kudu::Thread::Create("test", strings::Substitute("t$0", i), &RunAndAssign,
        writers_ready[i], readers_ready[i], &all_done, &threads_exiting,
        Substitute("$0", i), out_strings[i], &new_thread));
    threads.push_back(new_thread);
  }
