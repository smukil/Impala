...
TEST_F(TraceTest, TestChromeTracing) {
  ...
  tl->SetEnabled(CategoryFilter(CategoryFilter::kDefaultCategoryFilterString),
      TraceLog::RECORDING_MODE, TraceLog::RECORD_CONTINUOUSLY);
  ...
  for (int i = 0; i < kNumThreads; i++) {
    CHECK_OK(Thread::Create(
        "test", "gen-traces", &GenerateTraceEvents, i, kEventsPerThread, &threads[i]));
  }
...
TEST_F(TraceTest, TestTraceFromExitedThread) {
  ...
  CHECK_OK(Thread::Create("test", "gen-traces", &GenerateTraceEvents, 1, kNumEvents, &t));
  t->Join();
  tl->SetDisabled();
  string trace_json = TraceResultBuffer::FlushTraceLogToString();
  LOG(INFO) << trace_json;
...
void GenerateTracesUntilLatch(
    AtomicInt<int64_t>* num_events_generated, CountDownLatch* latch) {
  while (latch->count()) {
    {
...
class TraceEventCallbackTest : public KuduTest {
 ...
 protected:
  ...
  static const Value* FindTraceEntry(
      const Value& trace_parsed, const char* string_to_match) {
    // Scan all items
    ...
    for (size_t i = 0; i < trace_parsed_count; i++) {
      ...
      for (Value::ConstMemberIterator it = value.MemberBegin(); it != value.MemberEnd();
           ++it) {
        if (it->name.IsString()
  ...
  void VerifyCollectedEvent(
      size_t i, unsigned phase, const string& category, const string& name) {
  ...
  static void Callback(MicrosecondsInt64 timestamp, char phase,
...
TEST_F(TraceEventCallbackTest, TraceEventCallback) {
  ...
  TRACE_EVENT_INSTANT0("all", "after callback removed", TRACE_EVENT_SCOPE_GLOBAL);
  ...
  for (size_t i = 1; i < collected_events_timestamps_.size(); i++) {
    EXPECT_LE(collected_events_timestamps_[i - 1], collected_events_timestamps_[i]);
  }
}

TEST_F(TraceEventCallbackTest, TraceEventCallbackWhileFull) {
  TraceLog::GetInstance()->SetEnabled(
      CategoryFilter("*"), TraceLog::RECORDING_MODE, TraceLog::RECORD_UNTIL_FULL);
  do {
...
TEST_F(TraceEventCallbackTest, TraceEventCallbackAndRecording1) {
  ...
  TraceLog::GetInstance()->SetEventCallbackEnabled(CategoryFilter("callback"), Callback);
  TRACE_EVENT_INSTANT0("recording", "no", TRACE_EVENT_SCOPE_GLOBAL);
  TRACE_EVENT_INSTANT0("callback", "yes", TRACE_EVENT_SCOPE_GLOBAL);
  TraceLog::GetInstance()->SetEnabled(
      CategoryFilter("recording"), TraceLog::RECORDING_MODE, TraceLog::RECORD_UNTIL_FULL);
...
class TraceEventSyntheticDelayTest : public KuduTest,
 ...
 public:
  TraceEventSyntheticDelayTest() { now_ = MonoTime::Min(); }

  virtual ~TraceEventSyntheticDelayTest() { ResetTraceEventSyntheticDelays(); }
...
TEST_F(TraceTest, TestVLogAndEchoToConsole) {
  TraceLog* tl = TraceLog::GetInstance();
  tl->SetEnabled(CategoryFilter(CategoryFilter::kDefaultCategoryFilterString),
      TraceLog::RECORDING_MODE, TraceLog::ECHO_TO_CONSOLE);
  FLAGS_v = 1;
  VLOG_AND_TRACE("test", 1) << "hello world";
  tl->SetDisabled();
}
