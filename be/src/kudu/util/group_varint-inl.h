...
inline const uint8_t* DecodeGroupVarInt32(
    const uint8_t* src, uint32_t* a, uint32_t* b, uint32_t* c, uint32_t* d) {
...
inline void DoExtractM128(
    __m128i results, uint32_t* a, uint32_t* b, uint32_t* c, uint32_t* d) {
...
inline const uint8_t* DecodeGroupVarInt32_SSE(
  ...
  __m128i shuffle_mask =
      _mm_load_si128(reinterpret_cast<__m128i*>(&SSE_TABLE[sel_byte * 16]));
...
inline const uint8_t* DecodeGroupVarInt32_SSE_Add(
    const uint8_t* src, uint32_t* ret, __m128i add) {
  DCHECK(SSE_TABLE_INITTED);
...
inline void AppendGroupVarInt32(
    faststring* s, uint32_t a, uint32_t b, uint32_t c, uint32_t d) {
...
inline void AppendGroupVarInt32Sequence(
    faststring* s, uint32_t frame_of_reference, uint32_t* ints, size_t size) {
  uint32_t* p = ints;
  while (size >= 4) {
    AppendGroupVarInt32(s, p[0] - frame_of_reference, p[1] - frame_of_reference,
        p[2] - frame_of_reference, p[3] - frame_of_reference);
    size -= 4;
    p += 4;
  }
