...
// the name of the metric, the entity it is attached to, its type, its units, and a
// description.
//
...
// ------------------------
// The other main type in the data model is the Metric Entity. The most basic entity is
// the
// "server" entity -- metrics such as memory usage, RPC rates, etc, are typically
// associated
...
// is FunctionGauges: see the class documentation below for a typical Gauge ownership
// pattern.
//
// Because the metrics entity holds a reference to the metric, this means that metrics
// will
// not be immediately destructed when your class instance publishing them is destructed.
// This is on purpose: metrics are retained for a configurable time interval even after
// they
// are no longer being published. The purpose of this is to allow monitoring systems,
// which
// only poll metrics infrequently (eg once a minute) to see the last value of a metric
// whose
...
//   identifier)");
//
// 3) At the top of your .cc file where you want to emit a metric, define the metric
// prototype:
//
...
//
// 5) In your class constructor, instantiate the metric based on the MetricEntity plumbed
// in:
//
...
// ------------------------------------------------------------
// Follow the same pattern as above, but also define a metric entity somewhere. For
// example:
//
...
// wrong entity type. This ensures that the metadata can fully describe the set of
// metric-entity
// relationships.
//
...
#define METRIC_DEFINE_gauge_string(entity, name, label, unit, desc, ...)               \
  ::kudu::GaugePrototype<std::string> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_bool(entity, name, label, unit, desc, ...)          \
  ::kudu::GaugePrototype<bool> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_int32(entity, name, label, unit, desc, ...)            \
  ::kudu::GaugePrototype<int32_t> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_uint32(entity, name, label, unit, desc, ...)            \
  ::kudu::GaugePrototype<uint32_t> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_int64(entity, name, label, unit, desc, ...)            \
  ::kudu::GaugePrototype<int64_t> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_uint64(entity, name, label, unit, desc, ...)            \
  ::kudu::GaugePrototype<uint64_t> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
#define METRIC_DEFINE_gauge_double(entity, name, label, unit, desc, ...)          \
  ::kudu::GaugePrototype<double> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))

#define METRIC_DEFINE_histogram(                                                     \
    entity, name, label, unit, desc, max_val, num_sig_digits)                        \
  ::kudu::HistogramPrototype METRIC_##name(                                          \
      ::kudu::MetricPrototype::CtorArgs(#entity, #name, label, unit, desc), max_val, \
      num_sig_digits)

// The following macros act as forward declarations for entity types and metric
// prototypes.
#define METRIC_DECLARE_entity(name) \
  extern ::kudu::MetricEntityPrototype METRIC_ENTITY_##name
#define METRIC_DECLARE_counter(name) extern ::kudu::CounterPrototype METRIC_##name
#define METRIC_DECLARE_gauge_string(name) \
  extern ::kudu::GaugePrototype<std::string> METRIC_##name
#define METRIC_DECLARE_gauge_bool(name) extern ::kudu::GaugePrototype<bool> METRIC_##name
...
#define METRIC_DECLARE_histogram(name) extern ::kudu::HistogramPrototype METRIC_##name

#if defined(__APPLE__)
#define METRIC_DEFINE_gauge_size(entity, name, label, unit, desc, ...)            \
  ::kudu::GaugePrototype<size_t> METRIC_##name(::kudu::MetricPrototype::CtorArgs( \
      #entity, #name, label, unit, desc, ##__VA_ARGS__))
...
struct MetricJsonOptions {
  MetricJsonOptions() : include_raw_histograms(false), include_schema_info(false) {}
...
class MetricEntityPrototype {
 public:
  ...
  // attributes.
  scoped_refptr<MetricEntity> Instantiate(MetricRegistry* registry, const std::string& id,
...
class MetricEntity : public RefCountedThreadSafe<MetricEntity> {
 public:
  ...
  scoped_refptr<AtomicGauge<T>> FindOrCreateGauge(
      const GaugePrototype<T>* proto, const T& initial_value);

  template <typename T>
  scoped_refptr<FunctionGauge<T>> FindOrCreateFunctionGauge(
      const GaugePrototype<T>* proto, const Callback<T()>& function);
 ...
 private:
  ...
  MetricEntity(
      const MetricEntityPrototype* prototype, std::string id, AttributeMap attributes);
  ~MetricEntity();
...
class Metric : public RefCountedThreadSafe<Metric> {
 public:
  ...
  virtual Status WriteAsJson(JsonWriter* writer, const MetricJsonOptions& opts) const = 0;
...
class MetricRegistry {
 public:
  ...
  scoped_refptr<MetricEntity> FindOrCreateEntity(const MetricEntityPrototype* prototype,
      const std::string& id, const MetricEntity::AttributeMap& initial_attrs);
...
class MetricPrototype {
 public:
  ...
  struct CtorArgs {
    CtorArgs(const char* entity_type, const char* name, const char* label,
        MetricUnit::Type unit, const char* description, uint32_t flags = 0)
...
class GaugePrototype : public MetricPrototype {
 public:
  ...
  scoped_refptr<AtomicGauge<T>> Instantiate(
      const scoped_refptr<MetricEntity>& entity, const T& initial_value) const {
  ...
  scoped_refptr<FunctionGauge<T>> InstantiateFunctionGauge(
      const scoped_refptr<MetricEntity>& entity, const Callback<T()>& function) const {
...
class Gauge : public Metric {
 public:
  ...
  virtual ~Gauge() {}
  virtual Status WriteAsJson(JsonWriter* w, const MetricJsonOptions& opts) const OVERRIDE;
...
class StringGauge : public Gauge {
 public:
  StringGauge(const GaugePrototype<std::string>* proto, std::string initial_value);
...
class AtomicGauge : public Gauge {
 public:
  ...
  T value() const { return static_cast<T>(value_.Load(kMemOrderRelease)); }
  virtual void set_value(const T& value) {
    value_.Store(static_cast<int64_t>(value), kMemOrderNoBarrier);
  }
  void Increment() { value_.IncrementBy(1, kMemOrderNoBarrier); }
  virtual void IncrementBy(int64_t amount) {
    value_.IncrementBy(amount, kMemOrderNoBarrier);
  }
  void Decrement() { IncrementBy(-1); }
  void DecrementBy(int64_t amount) { IncrementBy(-amount); }

 protected:
  virtual void WriteValue(JsonWriter* writer) const OVERRIDE { writer->Value(value()); }
...
class FunctionGaugeDetacher {
 ...
 private:
  ...
  void OnDestructor(const Closure& c) { callbacks_.push_back(c); }
...
class FunctionGauge : public Gauge {
 public:
  ...
  void AutoDetach(FunctionGaugeDetacher* detacher, T value = T()) {
    detacher->OnDestructor(Bind(&FunctionGauge<T>::DetachToConstant, this, value));
  }
...
class HistogramPrototype : public MetricPrototype {
 public:
  HistogramPrototype(const MetricPrototype::CtorArgs& args, uint64_t max_trackable_value,
      int num_sig_digits);
...
class Histogram : public Metric {
 public:
  ...
  Status GetHistogramSnapshotPB(
      HistogramSnapshotPB* snapshot, const MetricJsonOptions& opts) const;
...
// Measures a duration while in scope. Adds this duration to specified histogram on
// destruction.
