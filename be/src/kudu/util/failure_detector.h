...
// A failure detector is responsible for deciding whether a certain server is dead or
// alive.
class FailureDetector : public RefCountedThreadSafe<FailureDetector> {
 public:
  enum NodeStatus { DEAD, ALIVE };
  typedef std::unordered_map<std::string, NodeStatus> StatusMap;

  typedef Callback<void(const std::string& name, const Status& status)>
      FailureDetectedCallback;
  ...
  virtual Status Track(const std::string& name, const MonoTime& now,
...
class TimedFailureDetector : public FailureDetector {
 ...
 private:
  ...
  FailureDetector::NodeStatus GetNodeStatusUnlocked(
...
class RandomizedFailureMonitor {
 public:
  ...
  RandomizedFailureMonitor(
      uint32_t random_seed, int64_t period_mean_millis, int64_t period_std_dev_millis);
  ~RandomizedFailureMonitor();
  ...
  Status MonitorFailureDetector(
      const std::string& name, const scoped_refptr<FailureDetector>& fd);
