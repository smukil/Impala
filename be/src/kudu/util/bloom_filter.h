...
class BloomKeyProbe {
 public:
  ...
  explicit BloomKeyProbe(const Slice& key) : key_(key) {
    uint64_t h =
        util_hash::CityHash64(reinterpret_cast<const char*>(key.data()), key.size());
  ...
  uint32_t initial_hash() const { return h_1_; }
  ...
  uint32_t MixHash(uint32_t h) const { return h + h_2_; }
...
inline bool BloomFilter::MayContainKey(const BloomKeyProbe& probe) const {
  ...
  while (rem_hashes >= 2) {
    ...
    if (!BitmapTest(&bitmap_[0], bitpos1) || !BitmapTest(&bitmap_[0], bitpos2)) {
      return false;
    }
