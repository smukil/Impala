...
class StackTrace {
 public:
  ...
  void Reset() { num_frames_ = 0; }

  void CopyFrom(const StackTrace& s) { memcpy(this, &s, sizeof(s)); }
  ...
  // http://lists.nongnu.org/archive/html/libunwind-devel/2011-08/msg00054.html it is
  // "largely
  // async safe" and it would only deadlock in the case that you call it while a dynamic
  // library
  // load is in progress. We assume that dynamic library loads would almost always be
  // completed
  // very early in the application lifecycle, so for now, this is considered "async safe"
  // until
