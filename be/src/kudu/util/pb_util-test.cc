...
Status TestPBUtil::CreateKnownGoodContainerFileWithVersion(
    int version, CreateMode create, SyncMode sync) {
  ProtoContainerTestPB test_pb;
  test_pb.set_name(kTestKeyvalName);
  test_pb.set_value(kTestKeyvalValue);
...
Status TestPBUtil::BitFlipFileByteRange(
    const string& path, uint64_t offset, uint64_t length) {
  faststring buf;
...
TEST_P(TestPBContainerVersions, TestCorruption) {
  ...
  if (version_ == 1) {
    ...
    ASSERT_TRUE(s.IsIncomplete()) << "Should be incorrect size: " << path_ << ": "
                                  << s.ToString();
  }
  ASSERT_STR_CONTAINS(s.ToString(), "File size not large enough to be valid");
  ...
  if (version_ == 1) {
    ...
    ASSERT_TRUE(s.IsCorruption()) << "Should be invalid data length checksum: " << path_
                                  << ": " << s.ToString();
...
TEST_F(TestPBUtil, TestPopulateDescriptorSet) {
  {
    ...
    WritablePBContainerFile::PopulateDescriptorSet(pb.GetDescriptor()->file(), &protos);
...
void TestPBUtil::DumpPBCToString(const string& path, bool oneline_output, string* ret) {
  gscoped_ptr<RandomAccessFile> reader;
...
TEST_P(TestPBContainerVersions, TestDumpPBContainer) {
  const char* kExpectedOutput = "Message 0\n"
                                "-------\n"
                                "record_one {\n"
                                "  name: \"foo\"\n"
                                "  value: 0\n"
  ...
  const char* kExpectedOutputShort = "0\trecord_one { name: \"foo\" value: 0 } "
                                     "record_two { record { name: \"foo\" value: 0 } }\n"
                                     "1\trecord_one { name: \"foo\" value: 1 } "
                                     "record_two { record { name: \"foo\" value: 2 } }\n";
