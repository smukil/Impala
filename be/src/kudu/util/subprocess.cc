  ...
  *dir = opendir(kProcSelfFd);
  if (PREDICT_FALSE(dir == nullptr)) {
    return Status::IOError(
        Substitute("opendir(\"$0\") failed", kProcSelfFd), ErrnoToString(errno), errno);
  }
  return Status::OK();
}
...
Status ReadFdsFully(const string& progname, const vector<int>& fds, vector<string>* out) {
...
Subprocess::~Subprocess() {
  if (state_ == kRunning) {
    LOG(WARNING) << "Child process " << child_pid_ << "(" << JoinStrings(argv_, " ")
                 << ") "
...
Status Subprocess::Start() {
  ...
  unique_ptr<DIR, std::function<void(DIR*)>> fd_dir_closer(fd_dir, CloseProcFdDir);
  int ret = fork();
  if (ret == -1) {
...
Status Subprocess::GetExitStatus(int* exit_status, string* info_str) const {
  ...
  if (WIFEXITED(wait_status_)) {
    ...
    if (status == 0) {
      ...
      info = Substitute("$0: process exited with non-zero status $1", program_, status);
    }
    ...
    status = -1;
    info = Substitute(
        "$0: process reported unexpected wait status $1", program_, wait_status_);
    LOG(DFATAL) << info;
  }
  if (exit_status) {
    *exit_status = status;
...
Status Subprocess::Call(const vector<string>& argv, const string& stdin_in,
  ...
  if (!stdin_in.empty()
      && write(p.to_child_stdin_fd(), stdin_in.data(), stdin_in.size())
          < stdin_in.size()) {
    return Status::IOError(
        "Unable to write to child process stdin", ErrnoToString(errno), errno);
  }
...
Status Subprocess::DoWait(int* wait_status, WaitMode mode) {
  ...
  if (rc == -1) {
    return Status::RuntimeError("Unable to wait on child", ErrnoToString(errno), errno);
  }
