...
// Test gflags
DEFINE_string(
    test_nondefault_ff, "default", "Check if we track non defaults from flagfile");
DEFINE_string(
    test_nondefault_explicit, "default", "Check if we track explicitly set non defaults");
DEFINE_string(test_default_ff, "default", "Check if we track defaults from flagfile");
DEFINE_string(
    test_default_explicit, "default", "Check if we track explicitly set defaults");
...
TEST_F(FlagsTest, TestNonDefaultFlags) {
  ...
  CHECK_OK(WriteStringToFile(Env::Default(),
      Slice(flagfile_contents.data(), flagfile_contents.size()), flagfile_path));

  std::string flagfile_flag = strings::Substitute("--flagfile=$0", flagfile_path);
  int argc = 4;
  const char* argv[4] = {"some_executable_file", "--test_nondefault_explicit=nondefault",
      "--test_default_explicit=default", flagfile_flag.c_str()};

  char** casted_argv = const_cast<char**>(argv);
  ParseCommandLineFlags(&argc, &casted_argv, true);

  std::vector<const char*> expected_flags = {"--test_nondefault_explicit=nondefault",
      "--test_nondefault_ff=nondefault", flagfile_flag.c_str()};
