...
class MaintenanceOpStats {
 ...
 private:
  ...
  // The approximate amount of disk space that not doing this operation keeps us from
  // GCing from
  // the logs. May be 0.
...
class MaintenanceOp {
 public:
  ...
  void CancelAndDisable() { cancel_.Store(true); }
...
struct MaintenanceOpComparator {
  bool operator()(const MaintenanceOp* lhs, const MaintenanceOp* rhs) const {
...
class MaintenanceManager : public std::enable_shared_from_this<MaintenanceManager> {
 ...
 private:
  ...
  // Vector used as a circular buffer for recently completed ops. Elements need to be
  // added at
  // the completed_ops_count_ % the vector's size and then the count needs to be
  // incremented.
