...
class HighWaterMark {
 public:
  ...
  int64_t current_value() const { return current_value_.Load(kMemOrderNoBarrier); }

  // Return the max value.
  int64_t max_value() const { return max_value_.Load(kMemOrderNoBarrier); }
  ...
  bool TryIncrementBy(int64_t delta, int64_t max) {
    while (true) {
      ...
      if (PREDICT_TRUE(
              current_value_.CompareAndSet(old_val, new_val, kMemOrderNoBarrier))) {
        UpdateMax(new_val);
        return true;
      }
    }
 ...
 private:
  void UpdateMax(int64_t value) { max_value_.StoreMax(value, kMemOrderNoBarrier); }
