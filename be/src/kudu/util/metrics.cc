...
scoped_refptr<MetricEntity> MetricEntityPrototype::Instantiate(MetricRegistry* registry,
    const std::string& id, const MetricEntity::AttributeMap& initial_attrs) const {
...
MetricEntity::MetricEntity(
  ...
  : prototype_(prototype), id_(std::move(id)), attributes_(std::move(attributes)) {}
...
bool MatchMetricInList(const string& metric_name, const vector<string>& match_params) {
...
Status MetricEntity::WriteAsJson(JsonWriter* writer,
    const vector<string>& requested_metrics, const MetricJsonOptions& opts) const {
  bool select_all = MatchMetricInList(id(), requested_metrics);

  // We want the keys to be in alphabetical order when printing, so we use an ordered map
  // here.
...
Status MetricRegistry::WriteAsJson(JsonWriter* writer,
  ...
  entities
      .clear(); // necessary to deref metrics we just dumped before doing retirement scan.
...
scoped_refptr<MetricEntity> MetricRegistry::FindOrCreateEntity(
    const MetricEntityPrototype* prototype, const std::string& id,
...
// Metric
//
Metric::Metric(const MetricPrototype* prototype) : prototype_(prototype) {}
...
Status Gauge::WriteAsJson(JsonWriter* writer, const MetricJsonOptions& opts) const {
  writer->StartObject();
...
StringGauge::StringGauge(const GaugePrototype<string>* proto, string initial_value)
...
Status Counter::WriteAsJson(JsonWriter* writer, const MetricJsonOptions& opts) const {
  writer->StartObject();
...
HistogramPrototype::HistogramPrototype(const MetricPrototype::CtorArgs& args,
  ...
  CHECK(HdrHistogram::IsValidHighestTrackableValue(max_trackable_value)) << Substitute(
      "Invalid max trackable value on histogram $0: $1", args.name_, max_trackable_value);
...
Status Histogram::WriteAsJson(JsonWriter* writer, const MetricJsonOptions& opts) const {
