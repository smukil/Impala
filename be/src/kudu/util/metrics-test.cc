...
METRIC_DEFINE_gauge_uint64(
    test_entity, fake_memory_usage, "Memory Usage", MetricUnit::kBytes, "Test Gauge 1");
...
TEST_F(MetricsTest, SimpleAtomicGaugeTest) {
  ...
  ASSERT_EQ(
      METRIC_fake_memory_usage.description(), mem_usage->prototype()->description());
...
METRIC_DEFINE_gauge_int64(
    test_entity, test_func_gauge, "Test Gauge", MetricUnit::kBytes, "Test Gauge 2");
...
METRIC_DEFINE_gauge_uint64(test_entity, counter_as_gauge, "Gauge exposed as Counter",
    MetricUnit::kBytes, "Gauge exposed as Counter", EXPOSE_AS_COUNTER);
...
TEST_F(MetricsTest, JsonPrintTest) {
  ...
  out.str("");
  ASSERT_OK(
      entity_->WriteAsJson(&writer, {"not_a_matching_metric"}, MetricJsonOptions()));
  ASSERT_EQ("", out.str());
}
