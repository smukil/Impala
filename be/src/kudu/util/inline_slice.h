...
class InlineSlice {
 private:
  ...
  static_assert(
      STORAGE_SIZE <= 256, "InlineSlice storage size must be less than 256 bytes");
    ...
    if (len <= kMaxInlineData) {
      if (ATOMIC) {
        ...
        base::subtle::Acquire_Store(
            reinterpret_cast<volatile AtomicWord*>(buf_), bit_cast<uintptr_t>(dptr));
      } else {
        buf_[0] = len;
      }
      ...
      // TODO: if already indirect and the current storage has enough space, just reuse
      // that.

      // Set up the pointed-to data before setting a pointer to it. This ensures that
      // readers
      // never see a pointer to an invalid region (i.e one without a proper length
      // header).
 ...
 private:
  struct DiscriminatedPointer {
    ...
    bool is_indirect() const { return discriminator == 0xff; }
  };
  ...
  DiscriminatedPointer LoadValue() const {
    if (ATOMIC) {
      ...
      uintptr_t ptr_val =
          base::subtle::Acquire_Load(reinterpret_cast<volatile const AtomicWord*>(buf_));
  ...
  void set_ptr(void* ptr) {
    ...
    if (ATOMIC) {
      ...
      base::subtle::Release_Store(reinterpret_cast<volatile AtomicWord*>(buf_), to_store);
    } else {
