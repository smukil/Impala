...
TraceEventSyntheticDelay* TraceEventSyntheticDelay::Lookup(const std::string& name) {
  return TraceEventSyntheticDelayRegistry::GetInstance()->GetOrCreateDelay(name.c_str());
}

void TraceEventSyntheticDelay::Initialize(
    const std::string& name, TraceEventSyntheticDelayClock* clock) {
  name_ = name;
  clock_ = clock;
}
...
void TraceEventSyntheticDelay::Begin() {
  ...
  if (!target_duration_.Initialized()) return;

  MonoTime start_time = clock_->Now();
  {
    MutexLock lock(lock_);
    if (++begin_count_ != 1) return;
...
void TraceEventSyntheticDelay::End() {
  ...
  {
    MutexLock lock(lock_);
    if (!begin_count_ || --begin_count_ != 0) return;
    end_time = end_time_;
  }
  if (end_time.Initialized()) ApplyDelay(end_time);
}
...
MonoTime TraceEventSyntheticDelay::CalculateEndTimeLocked(const MonoTime& start_time) {
...
TraceEventSyntheticDelayRegistry* TraceEventSyntheticDelayRegistry::GetInstance() {
  return Singleton<TraceEventSyntheticDelayRegistry>::get();
}

TraceEventSyntheticDelayRegistry::TraceEventSyntheticDelayRegistry() : delay_count_(0) {}
...
TraceEventSyntheticDelay* TraceEventSyntheticDelayRegistry::GetOrCreateDelay(
  ...
  for (int i = 0; i < delay_count; ++i) {
    if (!strcmp(name, delays_[i].name_.c_str())) return &delays_[i];
  }
  ...
  DCHECK(delay_count < kMaxSyntheticDelays) << "must increase kMaxSyntheticDelays";
  if (delay_count >= kMaxSyntheticDelays) return &dummy_delay_;
...
ScopedSyntheticDelay::ScopedSyntheticDelay(const char* name, AtomicWord* impl_ptr)
...
kudu::debug::TraceEventSyntheticDelay* GetOrCreateDelay(
    const char* name, AtomicWord* impl_ptr) {
  ...
  if (!delay_impl) {
    delay_impl =
        kudu::debug::TraceEventSyntheticDelayRegistry::GetInstance()->GetOrCreateDelay(
            name);
    base::subtle::Release_Store(impl_ptr, reinterpret_cast<AtomicWord>(delay_impl));
  }
  return delay_impl;
}
