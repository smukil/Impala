...
#define TRACE_EVENT_INSTANT_ETW(name, id, extra) \
  base::debug::TraceLog::AddTraceEventEtw(       \
      TRACE_EVENT_PHASE_INSTANT, name, reinterpret_cast<const void*>(id), extra)
...
class BASE_EXPORT TraceEvent {
 public:
  ...
  void Initialize(int thread_id, MicrosecondsInt64 timestamp,
  ...
  static void AppendValueAsJSON(unsigned char type, TraceValue value, std::string* out);
  ...
  const unsigned char* category_group_enabled() const { return category_group_enabled_; }
...
class BASE_EXPORT TraceBufferChunk {
 public:
  TraceBufferChunk(uint32 seq) : next_free_(0), seq_(seq) {}
...
class BASE_EXPORT TraceBuffer {
 public:
  ...
  virtual void ReturnChunk(size_t index, gscoped_ptr<TraceBufferChunk> chunk) = 0;
...
class BASE_EXPORT CategoryFilter {
 ...
 private:
  ...
  static bool IsEmptyOrContainsLeadingOrTrailingWhitespace(const std::string& str);

  void Initialize(const std::string& filter_string);
  void WriteString(const StringList& values, std::string* out, bool included) const;
...
class BASE_EXPORT TraceLog {
 public:
  ...
  void SetEnabled(const CategoryFilter& category_filter, Mode mode, Options options);
  ...
  // interface simple.
  typedef void (*EventCallback)(MicrosecondsInt64 timestamp, char phase,
      const unsigned char* category_group_enabled, const char* name, uint64_t id,
      int num_args, const char* const arg_names[], const unsigned char arg_types[],
      const uint64_t arg_values[], unsigned char flags);

  // Enable tracing for EventCallback.
  void SetEventCallbackEnabled(const CategoryFilter& category_filter, EventCallback cb);
  ...
  typedef kudu::Callback<void(
      const scoped_refptr<kudu::RefCountedString>&, bool has_more_events)>
      OutputCallback;
  ...
  TraceEventHandle AddTraceEvent(char phase, const unsigned char* category_group_enabled,
      const char* name, uint64_t id, int num_args, const char** arg_names,
      const unsigned char* arg_types, const uint64_t* arg_values,
      const scoped_refptr<ConvertableToTraceFormat>* convertable_values,
      unsigned char flags);
  TraceEventHandle AddTraceEventWithThreadIdAndTimestamp(char phase,
      const unsigned char* category_group_enabled, const char* name, uint64_t id,
  ...
  static void AddTraceEventEtw(
      char phase, const char* category_group, const void* id, const char* extra);
  static void AddTraceEventEtw(
      char phase, const char* category_group, const void* id, const std::string& extra);
  ...
  void SetWatchEvent(const std::string& category_name, const std::string& event_name,
 ...
 private:
  ...
  FRIEND_TEST(TraceEventTestFixture, TraceBufferRingBufferHalfIteration);
  FRIEND_TEST(TraceEventTestFixture, TraceBufferRingBufferFullIteration);
  ...
  bool CheckGeneration(int generation) const { return generation == this->generation(); }
  void UseNextTraceBuffer();

  MicrosecondsInt64 OffsetNow() const { return OffsetTimestamp(GetMonoTimeMicros()); }
