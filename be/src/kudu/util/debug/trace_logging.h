...
class TraceVLog {
 public:
  TraceVLog(const char* file, int line, const char* category, bool do_vlog)
    : sink_(category), google_msg_(file, line, google::GLOG_INFO, &sink_, do_vlog) {}

  std::ostream& stream() { return google_msg_.stream(); }
  ...
  class TraceLogSink : public google::LogSink {
   public:
    ...
    void send(google::LogSeverity severity, const char* full_filename,
      ...
      str_ = ToString(severity, base_filename, line, tm_time, message, message_len);
    }
    virtual ~TraceLogSink() {
      TRACE_EVENT_INSTANT1(category_, "vlog", TRACE_EVENT_SCOPE_THREAD, "msg", str_);
    }
