...
// pointer.
const char* g_category_groups[MAX_CATEGORY_GROUPS] = {"toplevel",
    "tracing already shutdown",
    "tracing categories exhausted; must increase MAX_CATEGORY_GROUPS", "__metadata"};
...
class TraceBufferRingBuffer : public TraceBuffer {
 public:
  TraceBufferRingBuffer(size_t max_chunks)
    ...
    for (size_t i = 0; i < max_chunks; ++i) recyclable_chunks_queue_[i] = i;
  }

  ~TraceBufferRingBuffer() { STLDeleteElements(&chunks_); }
  ...
  virtual void ReturnChunk(size_t index, gscoped_ptr<TraceBufferChunk> chunk) OVERRIDE {
  ...
  virtual size_t Capacity() const OVERRIDE { return max_chunks_ * kTraceBufferChunkSize; }

  virtual TraceEvent* GetEventByHandle(TraceEventHandle handle) OVERRIDE {
    if (handle.chunk_index >= chunks_.size()) return nullptr;
    TraceBufferChunk* chunk = chunks_[handle.chunk_index];
    if (!chunk || chunk->seq() != handle.chunk_seq) return nullptr;
 ...
 private:
  class ClonedTraceBuffer : public TraceBuffer {
   public:
    ...
    ~ClonedTraceBuffer() { STLDeleteElements(&chunks_); }
    ...
    virtual void ReturnChunk(size_t index, gscoped_ptr<TraceBufferChunk>) OVERRIDE {
      NOTIMPLEMENTED();
    }
    virtual bool IsFull() const OVERRIDE { return false; }
  ...
  size_t NextQueueIndex(size_t index) const {
    index++;
    if (index >= queue_capacity()) index = 0;
    return index;
  }
...
class TraceBufferVector : public TraceBuffer {
 public:
  ...
  ~TraceBufferVector() { STLDeleteElements(&chunks_); }
...
template <typename T>
void InitializeMetadataEvent(TraceEvent* trace_event, int thread_id,
    const char* metadata_name, const char* arg_name, const T& value) {
  if (!trace_event) return;

  int num_args = 1;
  unsigned char arg_type;
  uint64_t arg_value;
  ::trace_event_internal::SetTraceValue(value, &arg_type, &arg_value);
  trace_event->Initialize(thread_id, MicrosecondsInt64(0), MicrosecondsInt64(0),
      TRACE_EVENT_PHASE_METADATA, &g_category_group_enabled[g_category_metadata],
      metadata_name, ::trace_event_internal::kNoEventId, num_args, &arg_name, &arg_type,
      &arg_value, nullptr, TRACE_EVENT_FLAG_NONE);
}
...
class MarkFlagInScope {
 public:
  ...
  ~MarkFlagInScope() { base::subtle::Release_Store(dst_, 0); }
...
void TraceBufferChunk::Reset(uint32 new_seq) {
  for (size_t i = 0; i < next_free_; ++i) chunk_[i].Reset();
  next_free_ = 0;
  seq_ = new_seq;
}
...
gscoped_ptr<TraceBufferChunk> TraceBufferChunk::Clone() const {
  ...
  for (size_t i = 0; i < next_free_; ++i) cloned_chunk->chunk_[i].CopyFrom(chunk_[i]);
...
class TraceLog::OptionalAutoLock {
 public:
  explicit OptionalAutoLock(base::SpinLock& lock) : lock_(lock), locked_(false) {}
...
void MakeHandle(
    uint32 chunk_seq, size_t chunk_index, size_t event_index, TraceEventHandle* handle) {
  DCHECK(chunk_seq);
...
void TraceEvent::Initialize(int thread_id, MicrosecondsInt64 timestamp,
    MicrosecondsInt64 thread_timestamp, char phase,
    const unsigned char* category_group_enabled, const char* name, uint64_t id,
  ...
  for (i = 0; i < num_args; ++i) {
    ...
    if (arg_types_[i] == TRACE_VALUE_TYPE_CONVERTABLE) continue;
  ...
  if (alloc_size) {
    ...
    for (i = 0; i < num_args; ++i) {
      if (arg_types_[i] == TRACE_VALUE_TYPE_CONVERTABLE) continue;
      if (arg_is_copy[i]) CopyTraceEventParameter(&ptr, &arg_values_[i].as_string, end);
    }
...
void TraceEvent::UpdateDuration(
    const MicrosecondsInt64& now, const MicrosecondsInt64& thread_now) {
  DCHECK(duration_ == -1);
...
void TraceEvent::AppendValueAsJSON(
    unsigned char type, TraceEvent::TraceValue value, std::string* out) {
  switch (type) {
    ...
    case TRACE_VALUE_TYPE_DOUBLE: {
      ...
      if (MathLimits<double>::IsFinite(val)) {
        ...
        if (real.find('.') == std::string::npos && real.find('e') == std::string::npos
...
void TraceEvent::AppendAsJSON(std::string* out) const {
  ...
  DCHECK(!strchr(name_, '"'));
  StringAppendF(out, "{\"cat\":\"%s\",\"pid\":%i,\"tid\":%i,\"ts\":%" PRId64 ","
                     "\"ph\":\"%c\",\"name\":\"%s\",\"args\":{",
      TraceLog::GetCategoryGroupName(category_group_enabled_), process_id, thread_id_,
      time_int64, phase_, name_);
  ...
  *out += "}";
  ...
  if (phase_ == TRACE_EVENT_PHASE_COMPLETE) {
    ...
    if (duration != -1) StringAppendF(out, ",\"dur\":%" PRId64, duration);
...
void TraceResultBuffer::Collect(
    const scoped_refptr<RefCountedString>& s, bool has_more_events) {
  if (first_) {
...
class TraceBucketData {
 public:
  TraceBucketData(AtomicWord* bucket, const char* name, TraceSampleCallback callback);
  ~TraceBucketData();
...
class TraceSamplingThread {
 ...
 private:
  ...
  // be called.
  void RegisterSampleBucket(TRACE_EVENT_API_ATOMIC_WORD* bucket, const char* const name,
      TraceSampleCallback callback);
  // Splits a combined "category\0name" into the two component parts.
  static void ExtractCategoryAndName(
      const char* combined, const char** category, const char** name);
...
// static
void TraceSamplingThread::DefaultSamplingCallback(TraceBucketData* bucket_data) {
  TRACE_EVENT_API_ATOMIC_WORD category_and_name =
      TRACE_EVENT_API_ATOMIC_LOAD(*bucket_data->bucket);
  if (!category_and_name) return;
  ...
  TRACE_EVENT_API_ADD_TRACE_EVENT(TRACE_EVENT_PHASE_SAMPLE,
      TraceLog::GetCategoryGroupEnabled(category_group), name, 0, 0, nullptr, nullptr,
      nullptr, nullptr, 0);
}
...
void TraceSamplingThread::RegisterSampleBucket(TRACE_EVENT_API_ATOMIC_WORD* bucket,
    const char* const name, TraceSampleCallback callback) {
...
TraceBucketData::TraceBucketData(
    AtomicWord* bucket, const char* name, TraceSampleCallback callback)
...
TraceEvent* TraceLog::ThreadLocalEventBuffer::AddTraceEvent(TraceEventHandle* handle) {
  ...
  if (trace_event && handle) MakeHandle(chunk_->seq(), chunk_index_, event_index, handle);
...
TraceLog::TraceLog()
  : mode_(DISABLED),
    ...
    event_callback_category_filter_(CategoryFilter::kDefaultCategoryFilterString),
  ...
  // macros fast.
  ANNOTATE_BENIGN_RACE_SIZED(g_category_group_enabled, sizeof(g_category_group_enabled),
      "trace_event category enabled");
  for (int i = 0; i < MAX_CATEGORY_GROUPS; ++i) {
    ANNOTATE_BENIGN_RACE(&g_category_group_enabled[i], "trace_event category enabled");
  }
...
const char* TraceLog::GetCategoryGroupName(const unsigned char* category_group_enabled) {
  ...
  DCHECK(category_ptr >= category_begin
      && category_ptr
          < reinterpret_cast<uintptr_t>(g_category_group_enabled + MAX_CATEGORY_GROUPS))
...
void TraceLog::UpdateCategoryGroupEnabledFlag(int category_index) {
  ...
  if (mode_ == RECORDING_MODE && category_filter_.IsCategoryGroupEnabled(category_group))
...
void TraceLog::UpdateCategoryGroupEnabledFlags() {
  ...
  for (int i = 0; i < category_index; i++) UpdateCategoryGroupEnabledFlag(i);
}

void TraceLog::UpdateSyntheticDelaysFromCategoryFilter() {
  ResetTraceEventSyntheticDelays();
  const CategoryFilter::StringList& delays = category_filter_.GetSyntheticDelayValues();
  ...
  for (ci = delays.begin(); ci != delays.end(); ++ci) {
    ...
    TraceEventSyntheticDelay* delay = TraceEventSyntheticDelay::Lookup(tokens.front());
    tokens.pop_front();
    while (!tokens.empty()) {
...
const unsigned char* TraceLog::GetCategoryGroupEnabledInternal(
    const char* category_group) {
  DCHECK(!strchr(category_group, '"')) << "Category groups may not contain double quote";
  ...
  DCHECK(category_index < MAX_CATEGORY_GROUPS) << "must increase MAX_CATEGORY_GROUPS";
...
void TraceLog::GetKnownCategoryGroups(std::vector<std::string>* category_groups) {
  SpinLockHolder lock(&lock_);
  int category_index = base::subtle::NoBarrier_Load(&g_category_index);
...
void TraceLog::SetEnabled(
    const CategoryFilter& category_filter, Mode mode, Options options) {
  std::vector<EnabledStateObserver*> observer_list;
  {
    ...
    if (dispatching_to_observer_list_) {
      DLOG(ERROR) << "Cannot manipulate TraceLog::Enabled state from an observer.";
      return;
    }
    ...
    if (options & ENABLE_SAMPLING) {
      ...
      sampling_thread_->RegisterSampleBucket(&g_trace_state[0], "bucket0",
          Bind(&TraceSamplingThread::DefaultSamplingCallback));
      sampling_thread_->RegisterSampleBucket(&g_trace_state[1], "bucket1",
          Bind(&TraceSamplingThread::DefaultSamplingCallback));
      sampling_thread_->RegisterSampleBucket(&g_trace_state[2], "bucket2",
          Bind(&TraceSamplingThread::DefaultSamplingCallback));

      Status s = Thread::Create("tracing", "sampler", &TraceSamplingThread::ThreadMain,
          sampling_thread_.get(), &sampling_thread_handle_);
      if (!s.ok()) {
  ...
  for (const auto& observer : observer_list) observer->OnTraceLogEnabled();
...
void TraceLog::SetDisabledWhileLocked() {
...

  {
    ...
    // trace event.
    lock_.Unlock();
    for (const auto& observer : observer_list) observer->OnTraceLogDisabled();
    lock_.Lock();
  }
  dispatching_to_observer_list_ = false;
}
...
void TraceLog::RemoveEnabledStateObserver(EnabledStateObserver* listener) {
  auto it = std::find(
      enabled_state_observer_list_.begin(), enabled_state_observer_list_.end(), listener);
  if (it != enabled_state_observer_list_.end()) enabled_state_observer_list_.erase(it);
}
...
TraceEvent* TraceLog::AddEventToThreadSharedChunkWhileLocked(
  ...
  if (!thread_shared_chunk_) {
    ...
    if (check_buffer_is_full) CheckIfBufferIsFullWhileLocked();
  }
  if (!thread_shared_chunk_) return nullptr;

  size_t event_index;
  TraceEvent* trace_event = thread_shared_chunk_->AddTraceEvent(&event_index);
  if (trace_event && handle) {
    MakeHandle(
        thread_shared_chunk_->seq(), thread_shared_chunk_index_, event_index, handle);
  }
  return trace_event;
}
...
void TraceLog::CheckIfBufferIsFullWhileLocked() {
  DCHECK(lock_.IsHeld());
  if (logged_events_->IsFull()) SetDisabledWhileLocked();
}

void TraceLog::SetEventCallbackEnabled(
    const CategoryFilter& category_filter, EventCallback cb) {
  SpinLockHolder lock(&lock_);
  base::subtle::NoBarrier_Store(&event_callback_, reinterpret_cast<AtomicWord>(cb));
...
void TraceLog::Flush(const TraceLog::OutputCallback& cb) {
  if (IsEnabled()) {
    ...
    if (!cb.is_null()) cb.Run(empty_result, false);
...
void TraceLog::ConvertTraceEventsToTraceFormat(gscoped_ptr<TraceBuffer> logged_events,
    const TraceLog::OutputCallback& flush_output_callback) {
  if (flush_output_callback.is_null()) return;
  ...
  do {
    ...
    for (size_t i = 0; i < kTraceEventBatchChunks; ++i) {
      ...
      for (size_t j = 0; j < chunk->size(); ++j) {
        if (i > 0 || j > 0) json_events_str_ptr->data().append(",");
...
void TraceLog::FinishFlush(
    int generation, const TraceLog::OutputCallback& flush_output_callback) {
...
TraceEventHandle TraceLog::AddTraceEvent(char phase,
  ...
  return AddTraceEventWithThreadIdAndTimestamp(phase, category_group_enabled, name, id,
      thread_id, now, num_args, arg_names, arg_types, arg_values, convertable_values,
      flags);
}
...
TraceEventHandle TraceLog::AddTraceEventWithThreadIdAndTimestamp(char phase,
    const unsigned char* category_group_enabled, const char* name, uint64_t id,
    int thread_id, const MicrosecondsInt64& timestamp, int num_args,
    const char** arg_names, const unsigned char* arg_types, const uint64_t* arg_values,
    const scoped_refptr<ConvertableToTraceFormat>* convertable_values,
    unsigned char flags) {
  TraceEventHandle handle = {0, 0, 0};
  if (!*category_group_enabled) return handle;

  DCHECK(name);

  if (flags & TRACE_EVENT_FLAG_MANGLE_ID) id ^= process_id_hash_;
  ...
  if (base::subtle::NoBarrier_Load(&thr_info->is_in_trace_event_)) return handle;
  ...
  if (PREDICT_FALSE(!thread_local_event_buffer)) {
    ...
    base::subtle::NoBarrier_Store(reinterpret_cast<AtomicWord*>(&thr_info->event_buffer_),
  ...
  if (thread_id == static_cast<int>(Thread::UniqueThreadId())) {
    ...
    if (kudu_thr) {
      ...
      if (PREDICT_FALSE(new_name != g_current_thread_name && new_name && *new_name)) {
        ...
        if (existing_name == thread_names_.end()) {
          ...
          bool found = std::find(existing_names.begin(), existing_names.end(), new_name)
              != existing_names.end();
          if (!found) {
            if (existing_names.size()) existing_name->second.push_back(',');
  ...
  if (*category_group_enabled & (ENABLED_FOR_RECORDING | ENABLED_FOR_MONITORING)) {
    ...
    if (trace_event) {
      trace_event->Initialize(thread_id, now, thread_now, phase, category_group_enabled,
          name, id, num_args, arg_names, arg_types, arg_values, convertable_values,
          flags);
  ...
  if (PREDICT_FALSE(console_message.size())) LOG(ERROR) << console_message;

  if (PREDICT_FALSE(reinterpret_cast<const unsigned char*>(
                        base::subtle::NoBarrier_Load(&watch_category_))
    ...
    if (event_name_matches) {
      if (!watch_event_callback_copy.is_null()) watch_event_callback_copy.Run();
    }
  }

  if (PREDICT_FALSE(*category_group_enabled & ENABLED_FOR_EVENT_CALLBACK)) {
    EventCallback event_callback =
        reinterpret_cast<EventCallback>(base::subtle::NoBarrier_Load(&event_callback_));
    if (event_callback) {
      event_callback(now,
          phase == TRACE_EVENT_PHASE_COMPLETE ? TRACE_EVENT_PHASE_BEGIN : phase,
          category_group_enabled, name, id, num_args, arg_names, arg_types, arg_values,
          flags);
    }
  }
...
std::string TraceLog::EventToConsoleMessage(
    unsigned char phase, const MicrosecondsInt64& timestamp, TraceEvent* trace_event) {
  SpinLockHolder thread_info_lock(&thread_info_lock_);
  ...
  int thread_id = trace_event ? trace_event->thread_id() : Thread::UniqueThreadId();
  ...
  for (size_t i = 0; i < depth; ++i) log << "| ";

  if (trace_event) trace_event->AppendPrettyPrinted(&log);
  if (phase == TRACE_EVENT_PHASE_END)
...
void TraceLog::AddTraceEventEtw(
    char phase, const char* name, const void* id, const char* extra) {
#if defined(OS_WIN)
  TraceEventETWProvider::Trace(name, phase, id, extra);
#endif
  INTERNAL_TRACE_EVENT_ADD(
      phase, "ETW Trace Event", name, TRACE_EVENT_FLAG_COPY, "id", id, "extra", extra);
}

void TraceLog::AddTraceEventEtw(
    char phase, const char* name, const void* id, const std::string& extra) {
#if defined(OS_WIN)
  TraceEventETWProvider::Trace(name, phase, id, extra);
#endif
  INTERNAL_TRACE_EVENT_ADD(
...
void TraceLog::UpdateTraceEventDuration(const unsigned char* category_group_enabled,
    const char* name, TraceEventHandle handle) {
  PerThreadInfo* thr_info = thread_local_info_;
  if (!thr_info) {
    thr_info = SetupThreadLocalBuffer();
  }
  ...
  // ECHO_TO_CONSOLE is enabled: AddTraceEvent -> LOG(ERROR) ->
  // GpuProcessLogMessageHandler -> PostPendingTask -> TRACE_EVENT ...
  if (base::subtle::NoBarrier_Load(&thr_info->is_in_trace_event_)) return;
  MarkFlagInScope thread_is_in_trace_event(&thr_info->is_in_trace_event_);
  ...
  if (console_message.size()) LOG(ERROR) << console_message;
...
void TraceLog::SetWatchEvent(const std::string& category_name,
    const std::string& event_name, const WatchEventCallback& callback) {
  const unsigned char* category = GetCategoryGroupEnabled(category_name.c_str());
  SpinLockHolder lock(&lock_);
  base::subtle::NoBarrier_Store(&watch_category_, reinterpret_cast<AtomicWord>(category));
...
void TraceLog::AddMetadataEventsWhileLocked() {
  ...
  InitializeMetadataEvent(AddEventToThreadSharedChunkWhileLocked(nullptr, false), 0,
      "num_cpus", "number", base::NumCPUs());
#endif
  ...
  if (process_sort_index_ != 0) {
    InitializeMetadataEvent(AddEventToThreadSharedChunkWhileLocked(nullptr, false),
        current_thread_id, "process_sort_index", "sort_index", process_sort_index_);
  }
  ...
  if (process_labels_.size() > 0) {
    ...
    InitializeMetadataEvent(AddEventToThreadSharedChunkWhileLocked(nullptr, false),
        current_thread_id, "process_labels", "labels", JoinStrings(labels, ","));
  }

  // Thread sort indices.
  for (auto& sort_index : thread_sort_indices_) {
    if (sort_index.second == 0) continue;
    InitializeMetadataEvent(AddEventToThreadSharedChunkWhileLocked(nullptr, false),
        sort_index.first, "thread_sort_index", "sort_index", sort_index.second);
  }
  ...
  for (auto& name : thread_names_) {
    ...
    InitializeMetadataEvent(AddEventToThreadSharedChunkWhileLocked(nullptr, false),
        name.first, "thread_name", "name", name.second);
  }
}
...
TraceEvent* TraceLog::GetEventByHandleInternal(
  ...
  if (!handle.chunk_seq) return nullptr;
...
void TraceLog::UpdateProcessLabel(int label_id, const std::string& current_label) {
  if (!current_label.length()) return RemoveProcessLabel(label_id);
...
void TraceLog::RemoveProcessLabel(int label_id) {
  ...
  if (it == process_labels_.end()) return;
...
bool CategoryFilter::DoesCategoryGroupContainCategory(
    const char* category_group, const char* category) const {
  DCHECK(category);
  ...
  for (const string& category_group_token : pieces) {
    ...
    if (MatchPattern(category_group_token.c_str(), category)) return true;
  }
  return false;
}
...
void CategoryFilter::Initialize(const std::string& filter_string) {
  ...
  for (string category : tokens) {
    ...
    if (category.find(kSyntheticDelayCategoryFilterPrefix) == 0
      ...
      category = category.substr(strlen(kSyntheticDelayCategoryFilterPrefix),
...
void CategoryFilter::WriteString(
    const StringList& values, std::string* out, bool included) const {
  bool prepend_comma = !out->empty();
  int token_cnt = 0;
  for (const auto& value : values) {
    if (token_cnt > 0 || prepend_comma) StringAppendF(out, ",");
    StringAppendF(out, "%s%s", (included ? "" : "-"), value.c_str());
    ++token_cnt;
  }
}

void CategoryFilter::WriteString(const StringList& delays, std::string* out) const {
  bool prepend_comma = !out->empty();
  int token_cnt = 0;
  for (const auto& delay : delays) {
    if (token_cnt > 0 || prepend_comma) StringAppendF(out, ",");
    StringAppendF(out, "%s%s)", kSyntheticDelayCategoryFilterPrefix, delay.c_str());
    ++token_cnt;
  }
}
...
bool CategoryFilter::IsCategoryGroupEnabled(const char* category_group_name) const {
  ...
  for (ci = disabled_.begin(); ci != disabled_.end(); ++ci) {
    if (DoesCategoryGroupContainCategory(category_group_name, ci->c_str())) return true;
  }
  ...
  for (ci = excluded_.begin(); ci != excluded_.end(); ++ci) {
    if (DoesCategoryGroupContainCategory(category_group_name, ci->c_str())) return false;
  }
...
void CategoryFilter::Merge(const CategoryFilter& nested_filter) {
  ...
  if (HasIncludedPatterns() && nested_filter.HasIncludedPatterns()) {
    included_.insert(
        included_.end(), nested_filter.included_.begin(), nested_filter.included_.end());
  } else {
    included_.clear();
  }

  disabled_.insert(
      disabled_.end(), nested_filter.disabled_.begin(), nested_filter.disabled_.end());
  excluded_.insert(
      excluded_.end(), nested_filter.excluded_.begin(), nested_filter.excluded_.end());
  delays_.insert(
      delays_.end(), nested_filter.delays_.begin(), nested_filter.delays_.end());
}
...
const CategoryFilter::StringList& CategoryFilter::GetSyntheticDelayValues() const {
  return delays_;
}
...
ScopedTraceBinaryEfficient::ScopedTraceBinaryEfficient(
  ...
  if (*category_group_enabled_) {
    event_handle_ = TRACE_EVENT_API_ADD_TRACE_EVENT_WITH_THREAD_ID_AND_TIMESTAMP(
        ...
        static_cast<int>(kudu::Thread::UniqueThreadId()), GetMonoTimeMicros(), 0, nullptr,
