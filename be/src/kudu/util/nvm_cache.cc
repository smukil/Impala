...
struct LRUHandle {
  ...
  Slice value() const { return Slice(&kv_data[key_length], val_length); }

  uint8_t* val_ptr() { return &kv_data[key_length]; }
};
...
NvmLRUCache::NvmLRUCache(VMEM* vmp) : usage_(0), vmp_(vmp), metrics_(NULL) {
...
class ShardedLRUCache : public Cache {
 ...
 public:
  ...
  virtual void Free(PendingHandle* ph) OVERRIDE { vmem_free(vmp_, ph); }
};
