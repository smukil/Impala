...
METRIC_DEFINE_gauge_uint64(server, tcmalloc_contention_time, "TCMalloc Contention Time",
    kudu::MetricUnit::kMicroseconds,
...
class ContentionStacks {
 public:
  ...
  // the call have been flushed. However, new stacks can be added concurrently with this
  // call.
 ...
 private:
  ...
  bool CollectSample(
      uint64_t* iterator, StackTrace* s, int64_t* trip_count, int64_t* cycles);
  ...
  enum { kNumEntries = 1024, kNumLinearProbeAttempts = 4 };
...
void ContentionStacks::Flush(std::ostringstream* out, int64_t* dropped) {
  ...
  while (g_contention_stacks->CollectSample(&iterator, &t, &count, &cycles)) {
    *out << cycles << "\t" << count << " @ "
         << t.ToHexString(StackTrace::NO_FIX_CALLER_ADDRESSES) << std::endl;
  }
...
void RegisterSpinLockContentionMetrics(const scoped_refptr<MetricEntity>& entity) {
  ...
  entity->NeverRetire(METRIC_tcmalloc_contention_time.InstantiateFunctionGauge(
...
uint64_t GetSpinLockContentionMicros() {
  ...
  double micros =
      static_cast<double>(wait_cycles) / base::CyclesPerSecond() * kMicrosPerSecond;
